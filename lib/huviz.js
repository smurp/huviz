...
(function() {
  ...
  CommandController = (function() {
    function CommandController(huviz, container, hierarchy) {
      ...
      this.taxon_picker_box_parent = this.build_taxon_picker("Classes", this.node_pickers);
      this.add_clear_both(this.comdiv);
    ...
    CommandController.prototype.control_label = function(txt, what) {
      var outer;
    ...
    CommandController.prototype.select_the_initial_set = function() {
      this.huviz.pick_taxon("Thing", true);
      this.huviz.pick_taxon("Thing", false);
    };
    ...
    CommandController.prototype.build_predicate_picker = function(label) {
      var id, where;
      id = 'predicates';
      where = (label != null) && this.control_label(label) || this.comdiv;
    ...
    CommandController.prototype.handle_like_input = function(evt) {
      ...
      like_value = this.like_input[0][0].value;
      ...
      if (like_has_a_value) {
        if (this.liking_all_mode) {
          ...
          if (!this.is_verb_phrase_empty()) {
            this.set_immediate_execution_mode(false);
          }
        }
      } else {
        if (this.liking_all_mode) {
      ...
      this.doit_butt.on('click', (function(_this) {
        return function() {
          if (_this.update_command()) {
            _this.huviz.run_command(_this.command);
            _this.reset_editor();
  ...
  var CommandController, DC_subject, Edge, FOAF_Group, FOAF_Person, FOAF_name, GraphCommandLanguageCtrl, GreenerTurtle, Huviz, MANY_SPACES_REGEX, NAME_SYNS, Node, OWL_Class, OWL_ObjectProperty, OWL_Thing, OntoViz, OntologicallyGrounded, Orlando, PEEKING_COLOR, Predicate, RDFS_label, RDF_a, RDF_literal, RDF_object, RDF_type, Socrata, TYPE_SYNS, Taxon, TextCursor, UNDEFINED, XML_TAG_REGEX, angliciser, default_node_radius_policy, dist_lt, distance, gcl, has_predicate_value, has_type, hpad, id_escape, ids_to_show, is_a_main_node, is_node_to_always_show, is_one_of, node_radius_policies, start_with_http, uniquer, wpad,
    __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    __indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; },
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };
  ...
  Huviz = (function() {
    ...
    Huviz.prototype.truncate_labels_to = 40;
    ...
    Huviz.prototype.init_sets = function() {
      this.nodes = SortedSet().sort_on("id").named('all').labelled(this.human_term.all);
      ...
      this.embryonic_set.docs = "Nodes which are not yet complete are 'embryonic' and not yet in '" + this.nodes.label + "'.";
      this.chosen_set = SortedSet().named("chosen").labelled(this.human_term.chosen).isFlag().sub_of(this.nodes).sort_on("id");
      this.chosen_set.docs = "Nodes which the user has individually '" + this.chosen_set.label + "' to graph by clicking or dragging them.";
      this.chosen_set.cleanup_verb = 'shelve';
      this.selected_set = SortedSet().named("selected").labelled(this.human_term.selected).isFlag().sub_of(this.nodes).sort_on("id");
      this.selected_set.docs = "Nodes which have been '" + this.selected_set.label + "' using the class picker, ie which are highlighted.";
      this.selected_set.cleanup_verb = "unselect";
      this.shelved_set = SortedSet().sort_on("name").named("shelved").labelled(this.human_term.shelved).sub_of(this.nodes).isState();
      this.shelved_set.docs = "Nodes which are '" + this.shelved_set.label + "' on the green surrounding 'shelf'.";
      this.discarded_set = SortedSet().sort_on("name").named("discarded").labelled(this.human_term.discarded).sub_of(this.nodes).isState();
      this.discarded_set.docs = "Nodes which have been '" + this.discarded_set.label + "' so they will not be included in graphs.";
      this.discarded_set.cleanup_verb = "shelve";
      this.hidden_set = SortedSet().sort_on("id").named("hidden").labelled(this.human_term.hidden).sub_of(this.nodes).isState();
      this.hidden_set.docs = "Nodes which are '" + this.hidden_set.label + "' but can be pulled into graphs by other nodes.";
      this.hidden_set.cleanup_verb = "shelve";
      this.graphed_set = SortedSet().sort_on("id").named("graphed").labelled(this.human_term.graphed).sub_of(this.nodes).isState();
      this.graphed_set.docs = "Nodes which are included in the central graph.";
      this.graphed_set.cleanup_verb = "unchoose";
      this.pinned_set = SortedSet().sort_on("id").named('fixed').labelled(this.human_term.fixed).sub_of(this.nodes).isFlag();
      this.pinned_set.docs = "Nodes which are '" + this.pinned_set.label + "' to the canvas";
      this.pinned_set.cleanup_verb = "unpin";
      this.labelled_set = SortedSet().named("labelled").labelled(this.human_term.labelled).isFlag().sub_of(this.nodes).sort_on("id");
    ...
    Huviz.prototype.find_focused_node_or_edge = function() {
      var closest, closest_point, edge_changed, focus_threshold, last_focused_edge, last_focused_node, new_focused_edge, new_focused_idx, new_focused_node, node_changed, svg_node;
      if (this.dragging) {
        return;
      }
    ...
    Huviz.prototype.add_quad = function(quad) {
      ...
      if (quad.o.type === RDF_object) {
        ...
        if (subj_n.embryo && is_one_of(pid, NAME_SYNS)) {
          this.set_node_name(subj_n, quad.o.value);
          this.develop(subj_n);
        }
      }
    ...
    Huviz.prototype.set_node_name = function(node, name) {
      var full_name, len;
      full_name = name.replace(/^\s+|\s+$/g, '');
      node.full_name = full_name;
      len = this.truncate_labels_to;
      if (len > 0) {
        return node.name = full_name.substr(0, len);
      } else {
    ...
    Huviz.prototype.parseAndShowTTLStreamer = function(data, textStatus, callback) {
  ...
  Orlando = (function(_super) {
    ...
    Orlando.prototype.XXhuman_term = {
      all: 'All',
      chosen: 'Chosen',
      selected: 'Selected',
      shelved: 'Shelved',
