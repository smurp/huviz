...
(function(/*! Stitch !*/) {
  ...
  FiniteStateMachine = (function() {
    ...
    FiniteStateMachine.prototype.set_state = function(state) {
      var called;
      called = this.call_method_by_name('become__' + state);
      this.state = state;
      return called;
    };

    FiniteStateMachine.prototype.leave_state = function() {
      return this.call_method_by_name('leave__' + this.state);
    };
    ...
    FiniteStateMachine.prototype.make_noop_msg = function(trans_id, old_state, new_state) {
      return this.constructor.name + " had neither " + ("on__" + trans_id + " leave__" + old_state + " or become__" + new_state);
    };

    FiniteStateMachine.prototype.transit = function(trans_id) {
      var called, msg, target_id, throw_log_or_ignore, transition;
      if (this.transitions == null) {
        this.transitions = {};
      }
      if ((transition = this.transitions[trans_id])) {
        called = this.call_method_by_name('on__' + trans_id);
        msg = this.make_noop_msg(trans_id, this.state, target_id);
        called = this.leave_state() || called;
