...
}).call(this)({"angliciser": function(exports, require, module) {/*
...
(typeof exports !== 'undefined' && exports !== null ? exports : this).angliciser = angliciser;}, "coloredtreepicker": function(exports, require, module) {/*
 * decaffeinate suggestions:
...
const { TreePicker } = require('treepicker');
...
`;
  ...
  recolor() {
    ...
    const branch = this.elem[0][0].children[0];
  ...
  recolor_recurse_DOM(retval, recursor, branch, indent) {
    ...
    if (branch.children.length > 0) {
      for (let elem of Array.from(branch.children)) {
        if (elem != null) {
  ...
  summarize_kid_colors(id, color_list) {
    ...
    if (kids != null) {
      for (let kid_id of Array.from(kids)) {
...
}, "deprecated": function(exports, require, module) {/*
 * decaffeinate suggestions:
 * DS001: Remove Babel/TypeScript constructor workaround
 * DS102: Remove unnecessary code created because of implicit returns
 * DS207: Consider shorter variations of null checks
...
}, "edge": function(exports, require, module) {/*
 * decaffeinate suggestions:
 * DS102: Remove unnecessary code created because of implicit returns
 * DS206: Consider reworking classes to avoid initClass
 * DS207: Consider shorter variations of null checks
...
}, "editui": function(exports, require, module) {/*
 * decaffeinate suggestions:
 * DS001: Remove Babel/TypeScript constructor workaround
 * DS101: Remove unnecessary use of Array.from
 * DS102: Remove unnecessary code created because of implicit returns
...
const { FiniteStateMachine } = require('fsm');
const indexdDBstore = require('indexeddbstoragecontroller');

class EditController extends FiniteStateMachine {
  static initClass() {
  
    (typeof exports !== 'undefined' && exports !== null ? exports : this).EditController = EditController;
  }
  constructor(huviz) {
  ...
  set_predicate_selector() {
    ...
    if (this.huviz.predicate_set) {
      for (let predicate of Array.from(this.huviz.predicate_set)) {
  ...
  save_edit_form() {
    ...
    const assrtSave = new indexdDBstore.IndexedDBStorageController(this.huviz);
...
EditController.initClass();
}, "fsm": function(exports, require, module) {/*
 * decaffeinate suggestions:
 * DS207: Consider shorter variations of null checks
 * DS208: Avoid top-level this
 * Full docs: https://github.com/decaffeinate/decaffeinate/blob/master/docs/suggestions.md
...
const gcl = require('graphcommandlanguage');
const { ColoredTreePicker } = require('coloredtreepicker');
...
of the classes indicated.`,
  ...
  reset_command_history() {
    return Array.from(this.command_list).map((record) =>
  ...
  get_script_body_as_json() {
    ...
    for (let elem_and_cmd of Array.from(this.command_list)) {
      const { cmd } = elem_and_cmd;
    ...
    const replacer = function(key, value) {
      ...
      const retlist = [];
      if (key === 'subjects') {
        for (let node_or_id of Array.from(value)) {
          var obj;
          if (!node_or_id.id) {
      ...
      if (key === 'sets') {
        for (let set_or_id of Array.from(value)) {
  ...
  on_fastforward_click(forward_to_idx) {
    ...
    return (() => {
      ...
      while (this.command_idx0 < forward_to_idx) {
        result.push(this.on_forward_click());
      }
      return result;
    })();
  }
  ...
  show_succession_of_hints() {
    ...
    $(".hints.hint_set").show();
    for (let reminder of Array.from($(".hints > .a_hint"))) {
  ...
  handle_newpredicate(e) {
    const { pred_uri } = e.detail;
    const { parent_lid } = e.detail;
    const { pred_lid } = e.detail;
    let { pred_name } = e.detail;
    if (!Array.from(this.predicates_ignored).includes(pred_uri)) { // FIXME merge with predicates_to_ignore
      if (!Array.from(this.predicates_ignored).includes(pred_lid)) { // FIXME merge with predicates_to_ignore
  ...
  build_predicate_picker(label) {
    ...
    return this.predicates_JQElem.resizable({handles: 's'});
  }
  ...
  recolor_edges(evt) {
    let count = 0;
    return Array.from(this.huviz.all_set).map((node) =>
      (() => {
        const result = [];
        for (let edge of Array.from(node.links_from)) {
          count++;
          const pred_n_js_id = edge.predicate.id;
          result.push(edge.color = this.predicate_picker.get_color_forId_byName(pred_n_js_id, 'showing'));
        }
        return result;
      })());
  }
  ...
  on_taxon_clicked(taxonId, new_state, elem, args) {
    ...
    if (new_state === 'showing') {
      if (['mixed', 'unshowing', 'empty'].includes(old_state)) {
        if (!(Array.from(this.engaged_taxons).includes(taxonId))) {
  ...
  unselect_node_class(node_class) {
    ...
    return this.engaged_taxons = this.engaged_taxons.filter(eye_dee => eye_dee !== node_class);
  }
  ...
  make_verb_sets() {
    ...
    if (this.huviz.show_hunt_verb) {
      return this.verb_sets.push({hunt: this.huviz.human_term.hunt});
    }
  }
  ...
  stop_working() {
    ...
    return this.already_working = undefined;
  }
  show_working_on(cmd){
  ...
  disengage_all_verbs() {
    return Array.from(this.engaged_verbs).map((vid) =>
      this.disengage_verb(vid));
  }
  unselect_all_node_classes() {
    return (() => {
      const result = [];
      for (let nid of Array.from(this.engaged_taxons)) {
        this.unselect_node_class(nid);
        result.push(this.taxon_picker.set_direct_state(nid, 'unshowing'));
      }
      return result;
    })();
  }
  ...
  push_future_onto_history() {
    if (this.future_cmdArgs.length) {
      ...
      for (let cmdArgs of Array.from(this.future_cmdArgs)) {
  ...
  delete_script_command_by_idx(idx) {
    const elem_and_cmd = this.command_list.splice(idx, 1)[0];
    //alert("about to delete: " + elem_and_cmd.cmd.str)
    const elem = elem_and_cmd.elem[0];
    if (!elem || !elem[0]) {
      return;
    }
    const orphan = elem[0];
    const pops = orphan.parentNode;
  ...
  build_command() {
    ...
    if (this.engaged_verbs.length > 0) {
      for (let v of Array.from(this.engaged_verbs)) {
    ...
    const like_str = (this.like_input[0][0].value || "").trim();
    if (like_str) {
      args.like = like_str;
    }
  ...
  prepare_command(cmd) {
    ...
    if (this.nextcommand_prompts_visible || true) { // NEEDED BY huviz_test.js
      ...
      if (this.command.verb_phrase_ready) {
        $(this.nextcommand_verb_phrase[0][0]).
      ...
      if (this.command.noun_phrase_ready) {
        $(this.nextcommand_noun_phrase[0][0]).
          addClass('nextcommand_prompt_ready').
          removeClass('nextcommand_prompt_unready');
      } else {
        $(this.nextcommand_noun_phrase[0][0]).
  ...
  build_verb_form() {
    ...
    return Array.from(this.verb_sets).map((vset) =>
  ...
  get_verbs_overridden_by(verb_id) {
    ...
    for (let vset of Array.from(this.verb_sets)) {
      if (vset[verb_id]) {
        for (let vid in vset) {
          const label = vset[vid];
          if (!(Array.from(override).includes(vid)) && (verb_id !== vid)) {
            override.push(vid);
          }
        }
      }
  ...
  */
  ...
  engage_verb(verb_id, transient) {
    ...
    for (let vid of Array.from(this.engaged_verbs)) {
      if (Array.from(overrides).includes(vid)) {
        this.disengage_verb(vid);
      }
    }
    if (!(Array.from(this.engaged_verbs).includes(verb_id))) {
  ...
  populate_all_set_docs() {
    return (() => {
      ...
      for (let id in this.huviz.selectable_sets) {
        ...
        if (a_set.docs != null) {
          result.push(this.set_picker.set_title(id, a_set.docs));
        } else {
  ...
  make_sets_proposable() {
    ...
    return (() => {
      ...
      for (let id in this.huviz.selectable_sets) {
        const a_set = this.huviz.selectable_sets[id];
        result.push(make_listeners(id, a_set));
      }
      return result;
    })();
  }
  ...
  on_set_picked(set_id, new_state) {
    ...
    if (new_state === 'showing') {
      ...
      this.taxon_picker.unshield();
      const XXXcmd = new gcl.GraphCommand(this.huviz, {
          verbs: ['unselect'],
          sets: [this.chosen_set.id]
        });
      this.disengage_all_sets();
  ...
  clear_all_sets() {
    ...
    for (let set_key in this.the_sets.all_set[1]) {
      ...
      if (Array.from(skip_sets).includes(set_key)) {
        continue;
      }
      const the_set = this.huviz[set_key];
      const { cleanup_verb } = the_set;
      const cmd = new gcl.GraphCommand(this.huviz, {
        verbs: [cleanup_verb],
        sets: [the_set.id]
      });
      this.huviz.run_command(cmd);
...
}, "graphcommandlanguage": function(exports, require, module) {/*
 * decaffeinate suggestions:
 * DS101: Remove unnecessary use of Array.from
 * DS102: Remove unnecessary code created because of implicit returns
 * DS205: Consider reworking code to avoid use of IIFEs
...
class GCLTest {
  ...
  perform() {
    ...
    for (let exp of Array.from((this.spec.expectations != null ? this.spec.expectations : [] ))) {
      var got;
      console.log("exp",exp);
      try {
        got = eval(exp[0]);
...
class GCLTestSuite {
  ...
  run() {
    ...
    for (let spec of Array.from(this.suite)) {
      num++;
      let passed = false;
    ...
    for (let fail of Array.from(fails)) {
...
class GraphCommand {
  ...
  get_nodes() {
    ...
    if (this.subjects) {
      for (let node_spec of Array.from(this.subjects)) {
    ...
    if (this.sets) {
      for (let set of Array.from(this.sets)) { // set might be a SortedSet instance or a_set_id string
        var a_set;
  ...
  get_methods() {
    const methods = [];
    for (let verb of Array.from(this.verbs)) {
  ...
  execute() {
    ...
    if (regarding_required) {
      for (meth of Array.from(this.get_predicate_methods())) {
        iter = node => {
          for (let pred of Array.from(this.regarding)) {
      ...
      for (meth of Array.from(this.get_methods())) { // find the methods on huviz which implement each verb
        var callback;
        if (meth.callback) {
          ({ callback } = meth);
  ...
  get_pretty_verbs() {
    const l = [];
    for (let verb_id of Array.from(this.verbs)) {
  ...
  update_str() {
    ...
    if (this.sets != null) {
      const setLabels = [];
      for (let set of Array.from(this.sets)) {  // either a list of SortedSets or their ids
        var aSet;
        if (typeof set === 'string') {
      ...
      if (this.object_phrase) {
        ...
        if (this.except_subjects) {
          obj_phrase += ' except ' + angliciser(((() => {
            const result = [];
            for (subj of Array.from(this.subjects)) {               result.push(subj.lid);
            }
            return result;
          })()));
        }
        ...
        obj_phrase = angliciser(((() => {
          const result1 = [];
          for (subj of Array.from(this.subjects)) {             result1.push(subj.lid || subj);
          }
          return result1;
        })()));
      }
...
}, "greenerturtle": function(exports, require, module) {/*
 * decaffeinate suggestions:
 * DS102: Remove unnecessary code created because of implicit returns
 * DS205: Consider reworking code to avoid use of IIFEs
 * DS206: Consider reworking classes to avoid initClass
...
exports.GreenerTurtle = GreenerTurtle;}, "huviz": function(exports, require, module) {/*
 * decaffeinate suggestions:
 * DS001: Remove Babel/TypeScript constructor workaround
 * DS101: Remove unnecessary use of Array.from
 * DS102: Remove unnecessary code created because of implicit returns
...
//
let default_node_radius_policy, is_one_of, node_radius_policies;
const {
  angliciser
} = require('angliciser');
const {
  uniquer
} = require("uniquer"); // FIXME rename to make_dom_safe_id
const gcl = require('graphcommandlanguage');
...
} = require('indexeddbstoragecontroller');
const {
  Edge
} = require('edge');
const {
  GraphCommandLanguageCtrl
} = require('graphcommandlanguage');
const {
  GreenerTurtle
} = require('greenerturtle');
const {
  Node
} = require('node');
const {
...
const unpad_md = function(txt, pad) {
  ...
  for (let line of Array.from(in_lines)) {
...
var linearize = function(msgRecipient, streamoid) {
  if (streamoid.idx === 0) {
    return msgRecipient.postMessage({event: 'finish'});
  } else {
...
window.log_click = () => console.log("%cCLICK", "color:red;font-size:1.8em");
...
if (true) {
  ...
  is_one_of = (itm, array) => array.indexOf(itm) > -1;
}
...
var Huviz = (function() {
  ...
  let node_display_type = undefined;
  Huviz = class Huviz {
    static initClass() {
...
`;
      ...
      */
      ...
      this.prototype.domain2ldfServer = {
      ...
      //   show_cosmetic_tabs
      this.prototype.default_settings = [{
...
Summarize them as a hasAnnotation edge and enable the Annotation Inspector.`
  ...
  ${contents}
    ...
    get_mouse_point(d3_event) {
      if (d3_event == null) { d3_event = this.mouse_receiver[0][0]; }
    ...
    mousemove() {
      ...
      if (this.peeking_node != null) {
        ...
        if ((this.focused_node != null) && (this.focused_node !== this.peeking_node)) {
          ...
          for (let edge of Array.from(this.peeking_node.links_shown)) {
            if (Array.from(pair).includes(edge.source.id) && Array.from(pair).includes(edge.target.id)) {
    ...
    mouseup() {
      window.log_click();
      const d3_event = this.mouse_receiver[0][0];
      ...
      const drag_dist = distance(point, this.mousedown_point);
    ...
    render_node_info_box(info_node) {
      ...
      for (let name of Array.from(all_names)) {
        if (names_all_langs) {
      ...
      if (info_node._types.length > 1) {
        for (let node_type of Array.from(info_node._types)) {
      ...
      if (info_node.links_from.length > 0) {
        for (let link_from of Array.from(info_node.links_from)) {
...
`;
      ...
      if (info_node._colors) {
        ...
        for (color of Array.from(info_node._colors)) {
...
`; // """
    ...
    perform_current_command(node) {
      if (this.gclui.ready_to_perform()) {
        const cmd = new gcl.GraphCommand(this, {
...
with Shelved, Discarded, Graphed and Hidden.`;
    ...
    update_predicate_counts() {
      ...
      return (() => {
        const result = [];
        for (let a_set of Array.from(this.predicate_set)) {
          const name = a_set.lid;
          result.push(this.gclui.on_predicate_count_update(name, a_set.length));
        }
        return result;
      })();
    }
    ...
    update_set_counts() {
      return (() => {
        ...
        for (let name in this.selectable_sets) {
          ...
          result.push(this.gclui.on_set_count_update(name, a_set.length));
        }
        return result;
      })();
    }
      ...
      return (() => {
        ...
        // a label might be for a taxon or a predicate, so we must sort out which
          ...
          if (this.gclui.predicate_picker.id_to_name[term_id] != null) {
            result.push(this.gclui.predicate_picker.set_name_for_id(term_label, term_id));
          } else {
            result.push(undefined);
          }
        }
    ...
    do(args) {
      const cmd = new gcl.GraphCommand(this, args);
    ...
    DEPRECATED_init_node_radius_policy() {
      ...
      return (() => {
        ...
        for (let policy_name in node_radius_policies) {
          result.push(policy_picker.append("option").attr("value", policy_name).text(policy_name));
        }
        return result;
      })();
    }
    ...
    get_visible_subset(super_set) {
      ...
      const retlist = [];
      for (let node of Array.from(super_set)) {
    ...
    set_focused_edge(new_focused_edge) {
      ...
      if (this.focused_edge !== new_focused_edge) {
        ...
        if (this.focused_edge != null) {
          ...
          return this.text_cursor.continue(); //initialization (no proposed edge active)
        }
      }
    }
    ...
    set_cursor_for_verbs(verbs) {
      ...
      const text = [Array.from(verbs).map((verb) => this.human_term[verb])].join("\n");
    ...
    on_tick_change_current_command_if_warranted() {
      ...
      if (this.prior_node_and_state !== this.get_focused_node_and_its_state()) { // ie if it has changed
        if (this.gclui.engaged_verbs.length) {
          ...
          return this.gclui.prepare_command(
    ...
    apply_fisheye() {
      this.links_set.forEach(e => {
        if (!e.target.fisheye) { return e.target.fisheye = this.fisheye(e.target); }
      });

      if (this.use_svg) {
        return link.attr("x1", d => d.source.fisheye.x).attr("y1", d => d.source.fisheye.y).attr("x2", d => d.target.fisheye.x).attr("y2", d => d.target.fisheye.y);
      }
    }
    ...
    draw_edges_from(node) {
      ...
      const draw_n_n = {};
      for (e of Array.from(node.links_shown)) {
        let msg = "";
        //if e.source is node
          //continue
        if (e.source.embryo) {
      ...
      return (() => {
        const result = [];
        for (n_n in draw_n_n) {
          var edges_between = draw_n_n[n_n];
          var sway = 1;
          result.push((() => {
            const result1 = [];
            for (e of Array.from(edges_between)) {
            //console.log e
              var line_width;
    ...
    draw_walk_edge_from(node, edge, sway) {
      ...
      if (this.edgeIsOnWalkedPath(edge)) {
        ...
        const e = edge;
        if (directional_edge) {
          return this.draw_curvedline(e.source.fisheye.x, e.source.fisheye.y, e.target.fisheye.x,
    ...
    should_show_label(node) {
      return (node.labelled ||
          node.focused_edge ||
          (this.label_graphed && (node.state === this.graphed_set)) ||
          dist_lt(this.last_mouse_pos, node, this.label_show_range) ||
          ((node.name != null) && node.name.match(this.search_regex))); // FIXME make this a flag that gets updated ONCE when the regex changes not something deep in loop!!!
    }
    draw_labels() {
      if (this.use_svg) {
      ...
      if (this.use_canvas || this.use_webgl) {
        ...
        const label_node = node => {
          if (!this.should_show_label(node)) { return; }
          const {
            ctx
          } = this;
          ...
          if (!this.graphed_set.has(node) && this.draw_lariat_labels_rotated) {
            ...
            if (node_display_type === 'pills') {
              ...
              for (let i = 0; i < text_split.length; i++) {
                text = text_split[i];
                if (cuts && Array.from(cuts).includes(i)) {
    ...
    blank_screen() {
      if (this.use_canvas || this.use_webgl) { return this.clear_canvas(); }
    }
    ...
    position_nodes_by_packing() {
      ...
      return (() => {
        const result = [];
        while (++i < n) {
          result.push(q.visit(this.position_node_by_packing(this.graphed_set[i])));
        }
        return result;
      })();
    }
    ...
    draw_edge_labels() {
      ...
      if (this.show_edge_labels_adjacent_to_labelled_nodes) {
        return (() => {
          const result = [];
          for (let edge of Array.from(this.links_set)) {
            if (edge.target.labelled || edge.source.labelled) {
              result.push(this.draw_edge_label(edge));
            } else {
              result.push(undefined);
            }
          }
    ...
    restart() {
      if (this.use_svg) { this.svg_restart(); }
      this.force.start();
      if (!this.args.skip_log_tick) {
    ...
    add_node_ghosts(d) {
      if (this.use_webgl) { return d.gl = add_node(scene, d.x, d.y, 3, d.color); }
    }
    ...
    auto_discover_header(uri, digestHeaders, sendHeaders) {
      ...
      return $.ajax({
        ...
        beforeSend(xhr) {
          //console.log(xhr)
          return Array.from(sendHeaders).map((pair) =>
        ...
        success: (data, textStatus, request) => {
          ...
          console.table((Array.from(request.getAllResponseHeaders().split("\n")).map((line) => line.split(':'))));
          return (() => {
            const result = [];
            for (let header of Array.from(digestHeaders)) {
    ...
    discovery_triple_ingestor_N3(data, textStatus, request, discoArgs) {
      ...
      return parser.parse(data, (err, quad, pref) => {
        ...
        if (quadTester(quad)) {
          return Array.from(quadMunger(quad)).map((aQuad) =>
    ...
    discovery_triple_ingestor_GreenTurtle(data, textStatus, request, discoArgs) {
      ...
      for (let subj_uri in dataset.subjects) {
        ...
        for (let pred_id in frame.predicates) {
          ...
          for (let obj of Array.from(pred.objects)) {
            const quad = {
              s: frame.id,
              p: pred.id,
            ...
            if (quadTester(quad)) {
              for (let aQuad of Array.from(quadMunger(quad))) {
    ...
    discover_labels(aUrl) {
      const discoArgs = {
        ...
        quadTester: quad => {
          ...
          if (!(Array.from(NAME_SYNS).includes(quad.p))) {
            return false;
          }
          return true;
        },
    ...
    preset_discover_geonames_remaining() {
      let count = 0;
      for (let node of Array.from(this.nameless_set)) {
        const url = node.id;
...
`;
    ...
    discover_geoname_name(aUrl) {
      ...
      const userId = this.discover_geonames_as;
      const k2p = this.discover_geoname_key_to_predicate_mapping;
      const url = `http://api.geonames.org/hierarchyJSON?geonameId=${id}&username=${userId}`;
          ...
          if (json.status) {
            ...
            if (json.status.message) {
              msg = `<dt style="font-size:.9em;color:red">${json.status.message}</dt>` +
                this.discover_geoname_name_instructions;
              if (userId) {
                msg = `${userId} ${msg}`;
              }
            }
            ...
            if (['trying', 'looking'].includes(state_at_start)) {
              ...
              if (widget.state === 'looking') {
                if (this.discover_geonames_remaining > 0) {
                  ...
                  const again = () => this.discover_names('geonames.org');
    ...
    make_sparql_name_for_getty(uris, expansion, prefix) {
      ...
      if (uris.length === 1) { // so just match that one uri directly
        ...
        subj_constraint = "FILTER (?s IN (" +
          (Array.from(uris).map((uri) => this.deprefix(uri, prefix, expansion))).join(', ') + "))";
      }
      return `\
...
`; // """
    ...
    auto_discover_name_for(namelessUri) {
      ...
      for (let domainName in this.domain2ldfServer) {
        serverUrl = this.domain2ldfServer[domainName];
        args = {
          namelessUri,
          serverUrl
        };
    ...
    json_name_success_handler(data, textStatus, jqXHR, queryManager) {
      ...
      try {
        table = [];
        for (let resultJson of Array.from(data.results.bindings)) {
    ...
    make_name_query(uri, in_args) {
      ...
      const lines = [
        ...
        "WHERE {"];
      let pred_num = 0;
      for (let pred of Array.from(predicates)) {
        if (pred_num) {
          lines.push('  UNION');
        }
        pred_num++;
    ...
    run_ldf_name_query(args) {
      const {namelessUri} = args;
      args.query = "# " +
        ( args.comment || `run_ldf_name_query(${namelessUri})`) + "\n" +
    ...
    run_managed_query_ldf(args) {
      ...
      let {success_handler, error_callback, timeout, result_handler, serverUrl, query} = args;
      if (serverUrl == null) { serverUrl = "http://fragments.dbpedia.org/2016-04/en"; } // TODO what?
      const ldf_worker = new Worker('/comunica-ldf-client/ldf-client-worker.min.js');
      ...
      ldf_worker.onmessage = event => {
        ...
        switch (type) {
          case 'result':
            ...
            return result_handler.call(this, result, queryManager);
          case 'error':
            return queryManager.fatalError(d);
          case 'end':
            return queryManager.finishCounting();
          case 'queryInfo': case 'log':
  ...
  SERVICE wikibase:label {
    ...
    consider_suppressing_edge_and_ends_re_OA(edge) {
      ...
      if (edge.predicate.lid === 'exact') { // TODO ensure ids are urls then do the above...
        ...
        if (!(Array.from(this.hasAnnotation_targets).includes(edge.target))) {
    ...
    suppress_node_re_OA_if_appropriate(node) {
      ...
      for (let taxon of Array.from(node.taxons)) {
    ...
    in_OA_vocab(url) {
      ...
      return url.startsWith(OA_) || match;
    }
    ...
    try_to_set_node_type(node, type_uri) {
      ...
      if (!(Array.from(node._types).includes(type_lid))) {
        node._types.push(type_lid);
      }
      const prev_type = node.type;
      node.type = type_lid;
      if (prev_type !== type_lid) {
        return this.assign_types(node); // if 1 then more data shown
      }
    }
...
`;
      ...
      const make_success_handler = () => {
        return (data, textStatus, jqXHR, queryManager) => {
          ...
          for (let graph of Array.from(results)) {
            var label;
            if (graph.label != null) {
    ...
    sparqlQryInput_show() {
      ...
      return this.sparqlQryInput_JQElem.css({'color': 'inherit'} );
    }
...
`;
    ...
    add_nodes_from_SPARQL(json_data, subject, queryManager) {
      ...
      return (() => {
        const result = [];
        for (let node of Array.from(results)) {
          ...
          if (node_list_empty === 0) { // Add first node (because list is empty)
            ...
            for (let snode of Array.from(this.sparql_node_list)) {
          ...
          if (node_not_in_list) {
            this.sparql_node_list.push(q);
            node_not_in_list = false;
            result.push(this.add_quad(q, subject));
          } else {
            result.push(undefined);
          }
        }
    ...
    add_nodes_from_SPARQL_Worker(queryTarget, callback) {
      ...
      worker.addEventListener('message', e => {
        ...
        for (let quad of Array.from(e.data.results)) {
          //console.log quad
          this.add_quad(quad);
    ...
    dump_locations(srch, verbose, func) {
      ...
      return nodes.forEach((node, i) => {
        ...
        if (!func || verbose) { return this.dump_details(node); }
      });
    }
    ...
    make_nodes(g, limit) {
      ...
      return (() => {
        ...
        //continue  unless subj.match(ids_to_show)
          ...
          count++;
          if (limit && (count >= limit)) { break; } else {
            result.push(undefined);
          }
        }
        return result;
    ...
    discard_found_nodes() {
      this.nodes.forEach((node, i) => {
        if (node.name.match(search_regex)) { return this.discard(node); }
      });
      return this.restart();
    }
    ...
    wander__atFirst() {
      ...
      return Array.from(this.chosen_set).map((node) =>
    ...
    wander__atLast() {
      ...
      for (let node of Array.from(removed)) {
        this.unchoose(node);
    ...
    animate_hunt(array, sought_node, mid_node, prior_node, pos) {
      ...
      if (pos.done) {
        ...
        const cmd = new gcl.GraphCommand(this, cmdArgs);
        this.run_command(cmd);
        return this.clean_up_all_dirt_once();
      }
    }
    ...
    recolor_node(n, default_color) {
      ...
      if (this.color_nodes_as_pies && (n._types.length > 1)) {
        ...
        return (() => {
          const result = [];
          for (let taxon_id of Array.from(n._types)) {
            if (typeof(taxon_id) === 'string') {
              const color = this.get_color_for_node_type(n, taxon_id) || default_color;
              result.push(n._colors.push(color));
            } else {
              result.push(undefined);
            }
          }
    ...
    unflag_all_edges(node) {
      return Array.from(node.links_shown).map((edge) =>
        (edge.focused = false));
    }
    ...
    print_edge(edge) {
      ...
      return (() => {
        const result = [];
        for (let context of Array.from(edge.contexts)) {
    ...
    print(node) {
      this.clear_snippets();
      for (let edge of Array.from(node.links_shown)) {
        this.print_edge(edge);
      }
    }
    ...
    register_gclc_prefixes() {
      ...
      return (() => {
        ...
        for (let abbr in this.G.prefixes) {
          ...
          result.push(this.gclc.prefixes[abbr] = prefix);
        }
        return result;
      })();
    }
    ...
    ensure_datasets(preload_group, store_in_db) {
      ...
      return (() => {
        const result = [];
        for (let ds_rec of Array.from(preload_group.datasets)) {
        ...
        //  because then defaults are not ownProperty
          ...
          result.push(this.ensure_dataset(ds_rec, store_in_db));
        }
        return result;
      })();
    }
    ...
    preload_datasets() {
      ...
      if (this.args.preload) {
        for (var preload_group_or_uri of Array.from(this.args.preload)) {
    ...
    preload_endpoints() {
      ...
      if (this.args.preload_endpoints) {
        for (var preload_group_or_uri of Array.from(this.args.preload_endpoints)) {
    ...
    make_generic_query(in_args) {
      ...
      lines.push("WHERE {");
      let pattern = "  ";
      for (let term of Array.from('spo'.split(''))) {
    ...
    visualize_dataset_using_ontology(ignoreEvent, dataset, ontologies) {
      ...
      const endpoint_label_uri = this.endpoint_labels_JQElem.val();
      ...
      let alreadyCommands = ((this.gclui.command_list != null) && this.gclui.command_list.length);
    ...
    init_gclc() {
      ...
      for (let pid of Array.from(this.predicates_to_ignore)) {
...
href="${base}${this.git_commit_hash}">${this.git_commit_hash}</a>`; // """
    ...
    set_ontology_with_label(ontology_label) {
      ...
      for (let ont_opt of Array.from($(sel))) { // FIXME make this re-entrant
...
`;                       // for emacs syntax hilighting  ---> "
      ...
      const make_success_handler = () => {
        return (data, textStatus, jqXHR, queryManager) => {
          ...
          for (let label of Array.from(results)) {
            const this_result = {
    ...
    get_default_tab(id) {
      for (let tab of Array.from(this.default_tab_specs)) {
        if (tab.id === id) {
          return tab;
        }
      }
    ...
    make_tabs_html() {
      ...
      for (let t of Array.from(tab_specs)) {
        if (typeof(t) === 'string') {
    ...
    create_tabs() {
      ...
      this.addHTML(html);
      for (let pair of Array.from(jQElem_list)) {
    ...
    addDivWithIdAndClasses(id, classes, specialElem) {
      const html = `<div id="${sel_to_id(id)}" class="${classes}"></div>`;
      if (specialElem) {
    ...
    ensure_divs() {
      ...
      for (let key of Array.from(this.needed_divs)) {
        var sel;
    ...
    make_JQElems() {
      ...
      for (let key of Array.from(this.needed_JQElems)) {
        var sel;
    ...
    calculate_args(incoming_args) {
      ...
      if (args.create_tabs_adjacent_to_selector == null) { args.create_tabs_adjacent_to_selector = args.huviz_top_sel; }
      return args;
    }
...
`; // """ this comment is to help emacs coffeescript mode
    ...
    replace_human_term_spans(optional_class) {
      ...
      return (() => {
        ...
        for (let canonical in this.human_term) {
          ...
          result.push($(selector).text(human).addClass(optional_class));
        }
        return result;
      })();
    }
    ...
    init_settings_to_defaults() {
      ...
      for (let control_spec of Array.from(this.default_settings)) {
        for (let control_name in control_spec) {
          ...
          if (control.input != null) {
            ...
            if (control.input.type === 'select') {
              ...
              for (let k in control.input) {
                ...
                if (k === 'value') {
                  const old_val = this[control_name];
              ...
              if (control.input.type === 'checkbox') {
                const value = (control.input.checked != null);
                this.change_setting_to_from(control_name, value, undefined); //@[control_name].checked)
              }
            }
            ...
            const event_type = control.event_type ||
                (['checkbox','range','radio'].includes(control.input.type) && 'input') ||
                'change';
    ...
    adjust_settings_from_list_of_specs(setting_specs) {
      for (let setting_spec of Array.from(setting_specs)) {
        for (let setting_name in setting_spec) {
          ...
          if (control.input != null) {
            ...
            if (control.input.type === 'select') {
              for (let option of Array.from(control.options)) {
                if (option.selected) {
                  ({
                    value
                  } = option);
    ...
    adjust_settings_from_kv_list(kv_list) {
      return (() => {
        ...
        for (let setting in kv_list) {
          ...
          result.push(this.adjust_setting(setting, value));
        }
        return result;
      })();
    }
    ...
    on_change_nodes_pinnable(new_val, old_val) {
      if (!new_val) {
        if (this.graphed_set) {
          return Array.from(this.graphed_set).map((node) =>
            (node.fixed = false));
        }
      }
    }
    ...
    on_change_truncate_labels_to(new_val, old_val) {
      ...
      if (this.all_set) {
        for (let node of Array.from(this.all_set)) {
    ...
    init_from_settings() {
      ...
      return Array.from($(".settings input")).map((elem) => // so we can modify them in a loop
    ...
    call_on_dataset_loaded(uri) {
      return this.gclui.on_dataset_loaded({uri});
    }
    ...
    load_script_from_JSON(json) {
      ...
      for (let cmdArgs of Array.from(json)) {
        if (Array.from(cmdArgs.verbs).includes('load')) {
...
`;
    ...
    build_pfm_live_monitor(name) {
      const label = this.pfm_data[`${name}`]["label"];
      const monitor = `<div class='feedback_module'>${label}: <svg id='pfm_${name}' class='sparkline' width='200px' height='50px' stroke-width='1'></svg></div>`;
      return monitor;
    }
    ...
    pfm_update() {
      ...
      if (this.nodes) { noN = this.nodes.length; } else { noN = 0; }
      $("#noN").html(`${noN}`);
      ...
      if (this.endpoint_loader) { noOR = this.endpoint_loader.outstanding_requests; } else { noOR = 0; }
      $("#noOR").html(`${noOR}`);
      ...
      return (() => {
        ...
        for (let pfm_marker in this.pfm_data) {
          ...
          if (this.pfm_data[`${pfm_marker}`]["timed_count"] && (this.pfm_data[`${pfm_marker}`]["timed_count"].length > 0)) {
            ...
            if (this.pfm_data[`${pfm_marker}`]["timed_count"].length > 60) { this.pfm_data[`${pfm_marker}`]["timed_count"].shift(); }
            ...
            result.push(sparkline.sparkline(document.querySelector(`#pfm_${pfm_marker}`), this.pfm_data[`${pfm_marker}`].timed_count));
          } else if (this.pfm_data[`${pfm_marker}`]["timed_count"]) {
            result.push(this.pfm_data[`${pfm_marker}`]["timed_count"] = [0.01]);
          } else {
            result.push(undefined);
          }
        }
    ...
    if ((GreenerTurtle != null) && (this.turtle_parser === 'GreenerTurtle')) {
      ...
      return (() => {
        ...
        for (let subj_uri in this.raw_ontology.subjects) {
          var frame = this.raw_ontology.subjects[subj_uri];
          var subj_lid = uniquer(subj_uri);
          result.push((() => {
            const result1 = [];
            for (let pred_id in frame.predicates) {
              var pred = frame.predicates[pred_id];
              var pred_lid = uniquer(pred_id);
              result1.push((() => {
                const result2 = [];
                for (let obj of Array.from(pred.objects)) {
                  ...
                  if (pred_lid === 'domain') {
                    result2.push(ontology.domain[subj_lid] = obj_lid);
                  } else if (pred_lid === 'range') {
                    if ((ontology.range[subj_lid] == null)) {
                      ontology.range[subj_lid] = [];
                    }
                    if (!(Array.from(ontology.range).includes(obj_lid))) {
                      result2.push(ontology.range[subj_lid].push(obj_lid));
                    } else {
                      result2.push(undefined);
                    }
                  } else if (['subClassOf', 'subClass'].includes(pred_lid)) {
                    result2.push(ontology.subClassOf[subj_lid] = obj_lid);
                  } else if (pred_lid === 'subPropertyOf') {
                    result2.push(ontology.subPropertyOf[subj_lid] = obj_lid);
                  } else {
                    result2.push(undefined);
                  }
                }
...
var Socrata = (function() {
  ...
  Socrata = class Socrata extends Huviz {
    ...
    parseAndShowJSON(data) {
      ...
      return (() => {
        const result = [];
        for (var dataset of Array.from(data)) {
...
}, "node": function(exports, require, module) {/*
 * decaffeinate suggestions:
 * DS101: Remove unnecessary use of Array.from
 * DS102: Remove unnecessary code created because of implicit returns
 * DS206: Consider reworking classes to avoid initClass
