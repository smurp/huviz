...
(function() {
  ...
  CommandController = (function() {
    function CommandController(huviz, container, hierarchy) {
      ...
      this.cmdtitle = d3.select("#tabs-history").append('div').attr('class', 'control_label').html('Command History');
      this.cmdlist = d3.select("#tabs-history").append('div').attr('class', 'commandlist');
      this.oldcommands = this.cmdlist.append('div').attr('id', 'commandhistory');
    ...
    CommandController.prototype.select_the_initial_set = function() {
      var toggleEveryThing;
      toggleEveryThing = (function(_this) {
        return function() {
          return _this.huviz.toggle_taxon("Thing", false);
        };
      })(this);
    ...
    CommandController.prototype.add_predicate = function(pred_lid, parent_lid, pred_name) {
      return this.predicate_picker.add(pred_lid, parent_lid, pred_name, this.handle_on_predicate_clicked);
    };
    ...
    CommandController.prototype.add_taxon = function(class_id, parent_lid, class_name, taxon) {
      this.taxon_picker.add(class_id, parent_lid, class_name, this.handle_on_taxon_clicked);
      this.taxon_picker.recolor_now();
    ...
    CommandController.prototype.handle_on_taxon_clicked = function(id, new_state, elem) {
      this.start_working();
      return setTimeout((function(_this) {
        return function() {
          return _this.on_taxon_clicked(id, new_state, elem);
        };
      })(this));
    };
    ...
    CommandController.prototype.on_taxon_clicked = function(id, new_state, elem) {
      var because, class_name, cmd, hasVerbs, old_state, taxon, transiently;
      ...
      if (new_state === 'showing') {
        if (old_state === 'mixed' || old_state === 'unshowing' || old_state === 'empty') {
          if (!(__indexOf.call(this.engaged_taxons, id) >= 0)) {
            this.engaged_taxons.push(id);
          }
          cmd = new gcl.GraphCommand(this.huviz, {
            verbs: ['select'],
            classes: (function() {
      ...
      this.taxon_picker.style_with_kid_color_summary_if_needed(id);
      if (new_state === 'showing') {
        because = {
          taxon_added: id
        };
        if (hasVerbs) {
          if (!this.immediate_execution_mode) {
            this.engage_verb('unselect', (transiently = true));
          }
        } else {
          because.cleanup = (function(_this) {
            return function() {
      ...
      this.update_command();
      this.taxa_being_clicked_decrement();
    };
    ...
    CommandController.prototype.verb_descriptions = {
      ...
      walk: "Put nodes in the graph and pull connected nodes in followed by shelving of the nodes which had been pulled into the graph previously.",
    ...
    CommandController.prototype.build_depth = function() {
      this.depthdiv.text('Activate/Walk Depth:').classed("control_label activate_depth", true);
    ...
    CommandController.prototype.is_proposed = function() {
      return this.proposed_verb || this.proposed_set;
    };
  ...
  Huviz = (function() {
    ...
    Huviz.prototype.p_display = true;

    Huviz.prototype.p_tick_count = 0;

    Huviz.prototype.p_loaded_quad_count = 0;
    ...
    Huviz.prototype.draw_circle = function(cx, cy, radius, strclr, filclr, start_angle, end_angle) {
      var incl_cntr;
    ...
    Huviz.prototype.draw_pie = function(cx, cy, radius, strclr, filclrs) {
      ...
      if (num === 1) {
        this.draw_circle(cx, cy, radius, strclr, filclrs[0]);
        return;
      }
      arc = tau / num;
      start_angle = 0;
      ...
      for (_i = 0, _len = filclrs.length; _i < _len; _i++) {
        ...
        this.draw_circle(cx, cy, radius, strclr, filclr, end_angle, start_angle);
    ...
    Huviz.prototype.init_sets = function() {
      ...
      this.wasChosen_set.docs = "Nodes are marked wasChosen by walk__atFirst for later comparison with nowChosen.";
      this.nowChosen_set = SortedSet().named("nowChosen").sort_on("id").labelled("Now Graphed").isFlag();
      this.nowChosen_set.docs = "Nodes pulled in by @choose() are marked nowChosen for later comparison against wasChosen by walk__atLast.";
      ...
      this.nameless_set = SortedSet().named("nameless").sort_on("nameless_since").labelled(this.human_term.nameless).isFlag().sub_of(this.all_set);
    ...
    Huviz.prototype.draw_nodes = function() {
      ...
      if (this.use_canvas || this.use_webgl) {
        return this.graphed_set.forEach((function(_this) {
          return function(d, i) {
            var filclr, node_radius, pill_height, pill_width, rndng, stroke_color, x, y;
            ...
            if (_this.use_canvas) {
              ...
              if (node_display_type === 'pills') {
                ...
                _this.draw_pie(d.fisheye.x, d.fisheye.y, node_radius, stroke_color, _this.get_node_color_or_color_list(d));
              }
            }
            if (_this.use_webgl) {
    ...
    Huviz.prototype.tick = function() {
      ...
      if (this.p_display) {
        return this.performance_dashboard('tick');
      }
    };
      ...
      return $.ajax({
        ...
        success: (function(_this) {
          return function(json, textStatus, request) {
            var lastGeoname, name, quad;
            lastGeoname = json.geonames[json.geonames.length - 1 || 0];
            name = (lastGeoname || {}).name;
            quad = {
              s: aUrl.toString(),
              p: RDFS_label,
              o: {
            ...
            return _this.inject_discovered_quad_for(quad, aUrl);
          };
        })(this)
      });
    };
    ...
    Huviz.prototype.add_quad = function(quad, sprql_subj) {
      ...
      subj = null;
      if (this.p_display) {
        this.performance_dashboard('add_quad');
      }
    ...
    Huviz.prototype.remove_from_nameless = function(node) {
      this.nameless_set.remove(node);
      return delete node.nameless_since;
    };
    ...
    Huviz.prototype.add_to_nameless = function(node) {
      ...
      return this.nameless_set.push(node);
    };
    ...
    Huviz.prototype.load_endpoint_data_and_show = function(subject, callback) {
      ...
      if (this.p_display) {
        this.performance_dashboard('sparql_request');
      }
    ...
    Huviz.prototype.maximize_gclui = function() {
      ...
      return console.log("minimize the interface");
    };
    ...
    Huviz.prototype.dump_current_settings = function(post) {
      ...
      this.init_graph_controls_from_json();
      return this.on_change_graph_title_style("subliminal");
    };
    ...
    Huviz.prototype.init_graph_controls_from_json = function() {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        for (control_name in control_spec) {
          ...
          graph_control = this.graph_controls.append('span').attr('id', control_name).attr('class', 'graph_control');
  ...
  OntologicallyGrounded = (function(_super) {
    ...
    function OntologicallyGrounded() {
      this.performance_dashboard = __bind(this.performance_dashboard, this);
    ...
    OntologicallyGrounded.prototype.performance_dashboard = function(reason, note) {
      var class_count, classes, edge_count, item, loaded_quads, perform, predicates, request_target, sprql_current_requests, sprql_requests, sprql_total_requests, tick_count, tick_count_number, warning;
      warning = "";
      class_count = 0;
      if (reason === 'add_quad') {
        this.p_loaded_quad_count++;
      } else if (reason === 'tick') {
        tick_count_number = this.p_tick_count++;
      } else if (reason === 'sparql_request') {
        sprql_requests = this.endpoint_loader.outstanding_requests;
      ...
      perform = "<div class='feedback_module'>Number of Nodes: " + this.nodes.length + "</div>";
      edge_count = "<div class='feedback_module'>Number of Edges: " + this.edge_count + "</div>";
      predicates = "<div class='feedback_module'>Number of Predicates: " + this.predicate_set.length + "</div>";
      classes = "<div class='feedback_module'>Number of Classes: " + class_count + "</div>";
      loaded_quads = "<div class='feedback_module'>Triples Added: " + this.p_loaded_quad_count + "</div>";
      tick_count = "<div class='feedback_module'>Ticks in Session: " + tick_count_number + "</div>";
      sprql_total_requests = "<div class='feedback_module'>Total SPARQL Requests: " + this.p_total_sprql_requests + "</div>";
      sprql_current_requests = "<div class='feedback_module'><p>Outstanding SPARQL Requests: " + this.endpoint_loader.outstanding_requests + "</p><p>" + request_target + "</p></div>";
      return $("#performance_dashboard").html(perform + edge_count + predicates + classes + loaded_quads + tick_count + sprql_total_requests + sprql_current_requests + warning);
    };
