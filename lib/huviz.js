  ...
  EditController = (function() {
    function EditController(huviz) {
      ...
      if (this.userValid === true && !this.con) {
        ...
        document.getElementById("viscanvas").appendChild(this.con);
...
(function() {
  ...
  CommandController = (function() {
    function CommandController(huviz, container, hierarchy) {
      ...
      this.on_downloadscript_clicked = __bind(this.on_downloadscript_clicked, this);
      ...
      console.log("Window height: " + this.huviz.height);
    ...
    CommandController.prototype.make_command_history = function() {
      ...
      this.scriptDownloadButton = this.scriptPlayerControls.append('button').attr('title', 'save script as .txt').attr('style', 'margin-left:1em').on('click', this.on_downloadscript_txt_clicked);
      this.scriptDownloadButton.append('i').attr("class", "fa fa-download").append('span').text('.txt');
      this.scriptDownloadJsonButton = this.scriptPlayerControls.append('button').attr('title', 'save script as .json').attr('style', '').on('click', this.on_downloadscript_json_clicked);
      ...
      this.oldcommands = this.cmdlist.append('div').attr('id', 'commandhistory').style('max-height', "" + (this.huviz.height - 80) + "px");
      this.command_list = [];
    ...
    CommandController.prototype.get_script_body_as_json = function() {
      ...
      replacer = function(key, value) {
        var node, retlist, _j, _len1;
        if (key === 'subjects') {
          retlist = [];
          for (_j = 0, _len1 = value.length; _j < _len1; _j++) {
            node = value[_j];
            retlist.push(node.lid || node);
          }
          return nodelist;
        }
        return value;
    ...
    CommandController.prototype.on_downloadscript_json_clicked = function() {
      this.on_downloadscript_clicked('json');
    };

    CommandController.prototype.on_downloadscript_txt_clicked = function() {
      this.on_downloadscript_clicked('txt');
    };

    CommandController.prototype.on_downloadscript_clicked = function(txtOrJson) {
      var node, theHref, transientLink;
      transientLink = this.scriptPlayerControls.append('a');
      transientLink.text('script');
      this.lastScriptName = prompt("What would you like to call your saved script?", this.get_downloadscript_name(txtOrJson));
    ...
    CommandController.prototype.build_predicate_picker = function(label) {
      var extra_classes, id, needs_expander, squash_case, title, use_name_as_label, where;
      id = 'predicates';
      ...
      this.predicate_picker = new ColoredTreePicker(this.predicatebox, 'anything', (extra_classes = []), (needs_expander = true), (use_name_as_label = true), (squash_case = true));
    ...
    CommandController.prototype.make_predicate_proposable = function(pred_lid) {
      ...
      predicate_ctl.on('mouseenter', (function(_this) {
        return function() {
          ...
          console.log(_this.proposed_verb, "Selected regarding", _this.regarding);
    ...
    CommandController.prototype.handle_on_predicate_clicked = function(pred_id, new_state, elem) {
      this.start_working();
      return setTimeout((function(_this) {
        return function() {
          return _this.on_predicate_clicked(pred_id, new_state, elem);
        };
      })(this));
    };
    ...
    CommandController.prototype.build_taxon_picker = function(label, where) {
      ...
      this.taxon_picker = new ColoredTreePicker(this.taxon_box, 'Thing', (extra_classes = []), (needs_expander = true), (use_name_as_label = true), (squash_case = true));
    ...
    CommandController.prototype.build_depth = function() {
      ...
      this.depthdiv.style('display', 'inline-block');
  ...
  GraphCommand = (function() {
    ...
    GraphCommand.prototype.get_nodes = function() {
      var a_set, a_set_id, class_name, like_regex, n, node, node_spec, result_set, the_set, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _m, _n, _ref, _ref1, _ref2, _ref3;
      ...
      if (this.sets) {
        ...
        for (_m = 0, _len4 = _ref3.length; _m < _len4; _m++) {
          a_set_id = _ref3[_m];
          a_set = this.graph_ctrl.get_set_by_id(a_set_id);
    ...
    GraphCommand.prototype.get_methods = function() {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if (method) {
          method.build_callback = this.graph_ctrl["" + verb + "__build_callback"];
          method.callback = this.graph_ctrl["" + verb + "__atLast"];
          method.atFirst = this.graph_ctrl["" + verb + "__atFirst"];
          methods.push(method);
        } else {
    ...
    GraphCommand.prototype.execute = function() {
      var USE_ASYNC, atFirst, callback, errorHandler, iter, meth, node, nodes, reg_req, _i, _j, _k, _len, _len1, _len2, _ref, _ref1;
      ...
      if (reg_req) {
        ...
        this.graph_ctrl.load_with(this.data_uri, this.with_ontologies);
        ...
        for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
          ...
          iter = (function(_this) {
            return function(node) {
              var retval;
              retval = meth.call(_this.graph_ctrl, node, _this);
              return _this.graph_ctrl.tick();
            };
          })(this);
          if (nodes != null) {
            if (USE_ASYNC = false) {
              ...
              this.graph_ctrl.gclui.set;
              callback();
            }
          }
        }
      }
      this.graph_ctrl.clean_up_all_dirt_once();
      this.graph_ctrl.hide_state_msg();
      this.graph_ctrl.force.start();
    };
    ...
    GraphCommand.prototype.update_str = function() {
      var aSet, cmd_str, like_str, maybe_every, missing, more, obj_phrase, ready, regarding_phrase, regarding_required, setLabels, subj, verb, _i, _j, _len, _len1, _ref, _ref1;
  ...
  var BASE10, BASE57, CommandController, DC_subject, DragAndDropLoader, Edge, EditController, FOAF_Group, FOAF_Person, FOAF_name, GraphCommandLanguageCtrl, GreenerTurtle, Huviz, IndexedDBService, IndexedDBStorageController, MANY_SPACES_REGEX, NAME_SYNS, Node, OWL_Class, OWL_ObjectProperty, OWL_Thing, OntoViz, OntologicallyGrounded, Orlando, PEEKING_COLOR, PRIMORDIAL_ONTOLOGY, PickOrProvide, Predicate, RDFS_label, RDF_Class, RDF_a, RDF_literal, RDF_object, RDF_subClassOf, RDF_type, SKOS_prefLabel, Socrata, TYPE_SYNS, Taxon, TextCursor, UNDEFINED, XL_literalForm, XML_TAG_REGEX, angliciser, colorlog, convert, default_node_radius_policy, dist_lt, distance, escapeHtml, gcl, getPrefixedTypeSignature, getTypeSignature, has_predicate_value, has_type, hash, hpad, id_escape, ids_to_show, int_to_base, is_a_main_node, is_node_to_always_show, is_one_of, linearize, node_radius_policies, orlando_human_term, start_with_http, synthIdFor, tau, themeStyles, typeSigRE, unescape_unicode, unique_id, uniquer, wpad,
    __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    __indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; },
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };
  ...
  window.blurt = function(str, type, noButton) {
    ...
    if (!noButton) {
      return $('#blurtbox').append("<button id='blurt_close' class='sml_bttn' type='button'>close</button>");
    }
  };

  window.blurtDate = function() {
    return blurt(("" + new Date() + "").substr(0, 24), null, true);
  };
  ...
  Huviz = (function() {
    ...
    Huviz.prototype.mousemove = function() {
      var action, d3_event, edge, pair, the_node, _i, _len, _ref, _ref1, _ref2, _ref3;
    ...
    Huviz.prototype.render_node_info_box = function() {
      ...
      if (this.focused_node) {
        ...
        d3.select('#viscanvas').append('div').attr('id', this.focused_node.lid).attr('class', 'contextMenu').classed('temp', true).style('display', 'block').style('top', "" + d3.event.clientY + "px").style('left', "" + d3.event.clientX + "px").style('max-width', "" + max_width + "px").style('max-height', "" + max_height + "px").html(node_info);
    ...
    Huviz.prototype.draw_edges_from = function(node) {
      var directional_edge, draw_n_n, e, edge_width, edges_between, i, line_width, msg, n_n, num_edges, previous_node, s1, s2, source_is_path, sway, target_is_path, walk_node, _i, _len, _ref, _results;
      ...
      for (n_n in draw_n_n) {
        ...
        _results.push((function() {
          ...
          for (_j = 0, _len1 = edges_between.length; _j < _len1; _j++) {
            ...
            if (this.walk_path_set.length > 0) {
              _ref1 = this.walk_path_set;
              for (i = _k = 0, _len2 = _ref1.length; _k < _len2; i = ++_k) {
                walk_node = _ref1[i];
                if (this.prune_walk_nodes === "directional_path") {
                  s1 = e.source.lid;
                  s2 = e.target.lid;
                  previous_node = this.walk_path_set[i - 1];
                  if (s1 === previous_node && s2 === walk_node) {
                    directional_edge = 'forward';
                  }
                  if (s2 === previous_node && s1 === walk_node) {
                    directional_edge = 'backward';
                  }
                } else {
                  if (e.source.lid === walk_node) {
                    source_is_path = true;
                  }
                  if (e.target.lid === walk_node) {
                    target_is_path = true;
                  }
                }
              }
            ...
            if (directional_edge) {
              this.draw_curvedline(e.source.fisheye.x, e.source.fisheye.y, e.target.fisheye.x, e.target.fisheye.y, sway, "blue", e.contexts.length, 1, e, directional_edge);
              directional_edge = false;
            }
            if (source_is_path && target_is_path) {
              this.draw_curvedline(e.source.fisheye.x, e.source.fisheye.y, e.target.fisheye.x, e.target.fisheye.y, sway, "black", e.contexts.length, 1, e);
    ...
    Huviz.prototype.draw_nodes = function() {
      ...
      if (this.use_canvas || this.use_webgl) {
        return this.graphed_set.forEach((function(_this) {
          return function(d, i) {
            var filclr, node, node_radius, pill_height, pill_width, rndng, special_focus, stroke_color, x, y, _i, _len, _ref;
    ...
    Huviz.prototype.draw_labels = function() {
      ...
      if (this.use_canvas || this.use_webgl) {
        ...
        label_node = (function(_this) {
          return function(node) {
            var adjust_x, adjust_y, alpha, cart_label, ctx, cuts, fill, flip, i, label, line_height, node_font_size, outline, pill_height, pill_width, print_label, radians, radius, result, text, textAlign, text_split, x, y, _i, _len;
            ...
            if (node.focused_node || (node.focused_edge != null)) {
              if (node_display_type === 'pills') {
                ...
                if (node.state.id === "graphed") {
                  ...
                  if (_this.cartouches) {
                    _this.draw_cartouche(cart_label, node.fisheye.x, node.fisheye.y);
                  }
                }
    ...
    Huviz.prototype.draw_cartouche = function(label, x, y) {
      var alpha, fill, height, outline, radius, width;
      width = this.ctx.measureText(label).width;
      height = this.label_em * this.focused_mag * 16;
      radius = this.edge_x_offset;
      fill = renderStyles.pageBg;
      alpha = .8;
      outline = false;
      x = x + this.edge_x_offset;
      y = y - height;
      width = width + 2 * this.edge_x_offset;
      height = height + this.edge_x_offset;
      return this.rounded_rectangle(x, y, width, height, radius, fill, outline, alpha);
    };
    ...
    Huviz.prototype.draw_edge_label = function(edge) {
      ...
      height = this.label_em * this.focused_mag * 16;
      if (this.cartouches) {
        this.draw_cartouche(label, edge.handle.x, edge.handle.y);
      }
    ...
    Huviz.prototype.auto_discover = function(uri, force) {
      ...
      if (uri.startsWith("http://sws.geonames.org/") && this.discover_geonames_as) {
        return this.discover_geoname_name(aUrl);
      }
    };
    ...
    Huviz.prototype.get_container_width = function(pad) {
      ...
      return this.width = w_width - $("#tabs").width();
    };
    ...
    Huviz.prototype.walk__atFirst = function() {
      var node, _i, _len, _ref, _results;
      if (!this.wasChosen_set.clear()) {
        throw new Error("expecting wasChosen to be empty");
      }
    ...
    Huviz.prototype.walk__atLast = function() {
      var active_nodes, connected_path, current_selection, e, edge, graphed_node, i, keep_graphed, keep_node, lid, new_path, node, nodeId, node_lid, nowRollCall, remove, remove_list, removed, shownness, ungraphed, unique_keep, unique_remove, valid_path_nodes, wasRollCall, _i, _j, _k, _l, _len, _len1, _len10, _len11, _len12, _len2, _len3, _len4, _len5, _len6, _len7, _len8, _len9, _m, _n, _o, _p, _q, _r, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6, _s, _t, _u;
      wasRollCall = this.wasChosen_set.roll_call();
      nowRollCall = this.nowChosen_set.roll_call();
      if (this.focused_node) {
        current_selection = this.focused_node;
      } else if (this.selected_set[0]) {
        current_selection = this.selected_set[0];
      } else {
        current_selection = this.activated_set[0];
      }
      valid_path_nodes = [];
      active_nodes = [];
      if (!this.prune_walk_nodes) {
        this.prune_walk_nodes = $("#prune_walk_nodes :selected").val();
      }
      _ref = this.walk_path_set;
      for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
        nodeId = _ref[i];
        if (nodeId === current_selection.lid) {
          new_path = this.walk_path_set.slice(0, i);
          this.walk_path_set = new_path;
        }
      }
      _ref1 = this.all_set;
      for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
        node = _ref1[_j];
        _ref2 = this.walk_path_set;
        for (_k = 0, _len2 = _ref2.length; _k < _len2; _k++) {
          lid = _ref2[_k];
          if (node.lid === lid) {
            active_nodes.push(node);
            _ref3 = node.links_from;
      ...
      for (_n = 0, _len5 = valid_path_nodes.length; _n < _len5; _n++) {
        node_lid = valid_path_nodes[_n];
        if (current_selection.lid === node_lid) {
          connected_path = true;
        }
      }
      if (connected_path) {
        this.walk_path_set.push(current_selection.lid);
        connected_path = false;
        if (this.prune_walk_nodes === "directional_path" || this.prune_walk_nodes === "pruned_path") {
          ungraphed = [];
          keep_graphed = [];
          for (_o = 0, _len6 = active_nodes.length; _o < _len6; _o++) {
            node = active_nodes[_o];
            keep_graphed.push(node);
            _ref5 = current_selection.links_shown;
            for (_p = 0, _len7 = _ref5.length; _p < _len7; _p++) {
              edge = _ref5[_p];
              if (edge.source.lid !== node.lid) {
                keep_graphed.push(edge.source);
              }
              if (edge.target.lid !== node.lid) {
                keep_graphed.push(edge.target);
              }
            }
          }
          unique_keep = Array.from(new Set(keep_graphed));
          remove_list = [];
          _ref6 = this.graphed_set;
          for (_q = 0, _len8 = _ref6.length; _q < _len8; _q++) {
            graphed_node = _ref6[_q];
            remove = false;
            for (_r = 0, _len9 = unique_keep.length; _r < _len9; _r++) {
              keep_node = unique_keep[_r];
              if (graphed_node.lid === keep_node.lid) {
                remove = false;
                break;
              } else {
                remove = true;
          ...
          for (_s = 0, _len10 = remove_list.length; _s < _len10; _s++) {
            node = remove_list[_s];
            this.chosen_set.remove(node);
            this.selected_set.remove(node);
            this.graphed_set.remove(node);
            node.unselect();
            this.hide_node_links(node);
            this.update_state(node);
            shownness = this.update_showing_links(node);
          }
        }
      } else {
        removed = this.wasChosen_set.filter((function(_this) {
        ...
        for (_t = 0, _len11 = active_nodes.length; _t < _len11; _t++) {
          node = active_nodes[_t];
          removed.push(node);
        }
        for (_u = 0, _len12 = removed.length; _u < _len12; _u++) {
          node = removed[_u];
          this.unchoose(node);
          this.wasChosen_set.remove(node);
        }
      ...
      return console.log(this.walk_path_set);
    };

    Huviz.prototype.walk = function(chosen) {
      return this.choose(chosen);
    };
      ...
      if (!this.datasetDB && indexedDB) {
        ...
        request.onsuccess = (function(_this) {
          return function(e) {
            ...
            return _this.fill_dataset_menus_from_datasetDB('onsuccess');
          };
        })(this);
        ...
        return request.onupgradeneeded = (function(_this) {
          return function(e) {
            ...
            return objectStore.transaction.oncomplete = function(e) {
              _this.datasetDB = db;
              return _this.fill_dataset_menus_from_datasetDB('onupgradeneeded');
            };
          };
        })(this);
      }
    ...
    Huviz.prototype.ensure_dataset = function(dataset_rec, store_in_db) {
      var uri;
      uri = dataset_rec.uri;
      ...
      if (dataset_rec.isOntology) {
        if (this.ontology_loader) {
          this.ontology_loader.add_dataset(dataset_rec, store_in_db);
        }
      }
      if (this.dataset_loader && !dataset_rec.isEndpoint) {
        this.dataset_loader.add_dataset(dataset_rec, store_in_db);
      }
      if (dataset_rec.isEndpoint && this.endpoint_loader) {
        return this.endpoint_loader.add_dataset(dataset_rec, store_in_db);
      }
    };
    ...
    Huviz.prototype.fill_dataset_menus_from_datasetDB = function(why) {
        ...
        return function(why) {
          ...
          return function(event) {
            ...
            if (cursor) {
              ...
              if (!dataset_rec.isOntology && !dataset_rec.isEndpoint) {
    ...
    Huviz.prototype.init_dataset_menus = function() {
      var endpoint_selector, last_val;
      if (!this.dataset_loader && this.args.dataset_loader__append_to_sel) {
        this.dataset_loader = new PickOrProvide(this, this.args.dataset_loader__append_to_sel, 'Dataset', 'DataPP', false, false);
      }
      if (!this.ontology_loader && this.args.ontology_loader__append_to_sel) {
        this.ontology_loader = new PickOrProvide(this, this.args.ontology_loader__append_to_sel, 'Ontology', 'OntoPP', true, false);
      }
      if (!this.endpoint_loader && this.args.endpoint_loader__append_to_sel) {
        this.endpoint_loader = new PickOrProvide(this, this.args.endpoint_loader__append_to_sel, 'SPARQL Endpoint', 'EndpointPP', false, true);
      }
    ...
    Huviz.prototype.visualize_dataset_using_ontology = function(ignoreEvent, dataset, ontologies) {
      var data, endpoint_label_uri, onto;
      ...
      if (this.local_file_data) {
        this.read_data_and_show(data.value);
      } else {
        this.load_data_with_onto(data, onto);
      }
      this.update_browser_title(data);
      return this.update_caption(data.value, onto.value);
    };
    ...
    Huviz.prototype.reset_dataset_ontology_loader = function() {
      ...
      return $("#huvis_controls .unselectable").removeAttr("style", "display:none");
    };

    Huviz.prototype.update_dataset_ontology_loader = function() {
      var ugb;
      if (!((this.dataset_loader != null) && (this.ontology_loader != null) && (this.endpoint_loader != null))) {
    ...
    Huviz.prototype.update_endpoint_form = function(e) {
      ...
      if (e.currentTarget.value === '') {
        ...
        return $("#" + this.ontology_loader.uniq_id).children('select').prop('disabled', 'disabled');
      }
    };
    ...
    Huviz.prototype.update_caption = function(dataset_str, ontology_str) {
      ...
      return $("#ontology_watermark").text(ontology_str);
    };
    ...
    function Huviz(args) {
      ...
      this.walk__atLast = __bind(this.walk__atLast, this);
      this.walk__atFirst = __bind(this.walk__atFirst, this);
      ...
      $("#tabs").on("resize", this.updateWindow);
      $(this.viscanvas).bind("_splitpaneparentresize", this.updateWindow);
      $("#collapse_cntrl").click(this.minimize_gclui).on("click", this.updateWindow);
      $("#full_screen").click(this.fullscreen);
      $("#expand_cntrl").click(this.maximize_gclui).on("click", this.updateWindow);
      $("#tabs").on('click', '#blurt_close', this.close_blurt_box);
      $("#tabs").tabs({
        active: 0
      });
    ...
    Huviz.prototype.close_blurt_box = function() {
      return $('#blurtbox').remove();
    };

    Huviz.prototype.minimize_gclui = function() {
      $('#tabs').prop('style', 'visibility:hidden;width:0');
      return $('#expand_cntrl').prop('style', 'visibility:visible');
    };

    Huviz.prototype.maximize_gclui = function() {
      $('#tabs').prop('style', 'visibility:visible');
      return $('#maximize_cntrl').prop('style', 'visibility:hidden');
    };
    ...
    Huviz.prototype.init_graph_controls_from_json = function() {
      var a, control, control_name, control_spec, graph_control, input, k, label, old_val, option, v, value, _i, _len, _ref, _ref1, _ref2;
    ...
    Huviz.prototype.load_with = function(data_uri, ontology_uris) {
      ...
      return this.visualize_dataset_using_ontology({}, dataset, [ontology]);
    };
    ...
    Huviz.prototype.pfm_dashboard = function() {
      ...
      return $("#performance_dashboard").html(message + warning);
    };
  ...
  OntologicallyGrounded = (function(_super) {
    ...
    OntologicallyGrounded.prototype.read_ontology = function(ontology_uri) {
      return $.ajax({
        ...
        error: (function(_this) {
          return function(jqxhr, textStatus, errorThrown) {
            return _this.show_state_msg(errorThrown + " while fetching ontology " + ontology_uri);
          };
        })(this)
      });
    };
  ...
  PickOrProvide = (function() {
    ...
    function PickOrProvide(huviz, append_to_sel, label, css_class, isOntology, isEndpoint) {
      this.huviz = huviz;
    ...
    PickOrProvide.prototype.select_option = function(option) {
      ...
      if (cur_val !== this.last_val && !this.isOntology) {
        this.last_val = cur_val;
      }
      if (this.last_val !== new_val) {
        this.last_val = new_val;
        if (new_val) {
          return this.pick_or_provide_select.val(new_val);
        } else {
    ...
    PickOrProvide.prototype.add_uri = function(uri_or_rec) {
      ...
      if (dataset_rec.label === "") {
        dataset_rec.label = "Endpoint";
      }
      this.add_dataset(dataset_rec, true);
      return this.update_state();
    };

    PickOrProvide.prototype.add_local_file = function(file_rec) {
      var data_type, dataset_rec, uri;
      data_type = "local";
      ...
      this.add_dataset(dataset_rec, false);
      return this.update_state();
    };

    PickOrProvide.prototype.add_dataset = function(dataset_rec, store_in_db) {
      var uri;
      uri = dataset_rec.uri;
      if (store_in_db) {
        return this.huviz.add_dataset_to_db(dataset_rec, this.add_dataset_option);
      } else {
        return this.add_dataset_option(dataset_rec);
      }
    };
    ...
    PickOrProvide.prototype.add_group = function(grp_rec, which) {
      ...
      optgrp = $("<optgroup label=\"" + grp_rec.label + "\" id=\"" + (grp_rec.id || unique_id()) + "\"></optgroup>");
      if (which === 'prepend') {
  ...
  DragAndDropLoader = (function() {
    ...
    function DragAndDropLoader(huviz, append_to_sel, picker) {
      ...
      console.log(this.append_to_sel);
        ...
        return function(evt) {
          ...
          try {
            if (filename.match(/.(ttl|.nq)$/)) {
              _this.picker.add_local_file({
                uri: firstFile.name,
                opt_group: 'Your Own'
              });
              return _this.huviz.local_file_data = evt.target.result;
            } else {
...
(function() {
  ...
  TreePicker = (function() {
    function TreePicker(elem, root, extra_classes, needs_expander, use_name_as_label) {
      this.elem = elem;
