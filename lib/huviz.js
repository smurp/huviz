...
(function() {
  ...
  Huviz = (function() {
    ...
    Huviz.prototype.find_node_or_edge_closest_to_pointer = function() {
      var closest, closest_point, edge_changed, focus_threshold, last_focused_edge, last_focused_node, last_temp_edge, new_focused_edge, new_focused_idx, new_focused_node, node_changed, obj_type, q, seeking, svg_node;
      ...
      if (seeking === 'object_node') {
        if (this.editui.object_node !== this.object_node) {
          ...
          if (this.object_node != null) {
            obj_type = this.editui.predicate_input.value === 'literal' ? RDF_literal : RDF_object;
            q = {
              s: this.focused_node.id,
              p: this.editui.predicate_input.value || "anything",
              o: {
                type: obj_type,
                value: this.object_node.id
              }
            };
            if (!this.edit_quad || this.edit_quad.s !== q.s || this.edit_quad.p !== q.p || this.edit_quad.o.value || q.o.value) {
              if (this.edit_quad) {
                last_temp_edge = this.edit_quad.s + ' ' + this.edit_quad.p + ' ' + this.edit_quad.o.value;
                delete this.edges_by_id[last_temp_edge];
              }
              this.edit_quad = this.add_quad(q);
              console.log("I've made a new quad");
              console.log(this.edges_by_id);
            }
          }
        }
      }
