...
var SortedSet = function(){
    ...
    array._cmp_instrumented = function(a, b){
      ...
      var acase = array.case_insensitive && av.toUpperCase() || av,
          bcase = array.case_insensitive && bv.toUpperCase() || bv;
    ...
    array._verify_cmp = function(av, bv, retval) {
      var dir = ['less than', 'equal to', 'greater than'][retval+1],
          ...
          acase = array.case_insensitive && av.toUpperCase() || av,
          bcase = array.case_insensitive && bv.toUpperCase() || bv;
      if (sense == 'UNSQUASHED' &&
          acase != av &&
          av.toUpperCase() != av) {
    ...
    array.alter = function(itm, callback) {
      ...
      */
      ...
      console.log('alter()',itm.lid,"validate_sort_at() failed",current_idx);
      //console.table(array, ['lid','name','type']);
      //debugger;
	...
	//   Maintain a sorted array which acts like a set.
      ...
      if (window.WTF && window.WTF.includes(itm.lid) && array.id == 'shelved') {
        //console.table(array, ['lid','name','type']);
        //debugger
      }
	var c = array.binary_search(itm, true)
        ...
        // if c is an object instead, then c.idx contains the index where itm SHOULD be inserted
	...
	if (c > -1){  // it was found
	  duds = array.splice(c, 1);  // remove itm into the duds array (expecting length == 1)
          ...
          var msg = `remove() is failing because array.binary_search() failed to find ${itm.lid} in ${array.label}`;
          console.warn(msg);
          //console.table(array, ['lid','name','type']);
          //debugger;
  ...
  array.binary_search = function(sought, ret_ins_idx, callback){
	...
	var node = array[i];
	console.log('lid:',node.lid, 'name.toString():',node.name.toString(), 'node.name:',node.name);
      }
    }
...
var SortedSet_tests = function(verbose){
...
if (typeof module !== 'undefined' && module.exports) {
  ...
  module.exports.SortedSet_tests = SortedSet_tests;
}
...
(function() {
  ...
  UsernameWidget = (function(_super) {
    ...
    function UsernameWidget() {
      ...
      this.wrap("<span id=\"" + this.id + "\" style=\"border:2px solid; padding:2px\"></span>");
      this.inputElem.setAttribute('style', 'border:none');
  ...
  Huviz = (function() {
    ...
    Huviz.prototype.add_quad = function(quad, sprql_subj) {
      var cntx_n, ctxid, edge, isLiteral, is_type, literal_node, make_edge, newsubj, objId, objKey, objVal, obj_n, pred_n, pred_uri, simpleType, subj, subj_lid, subj_n, subj_uri, use_thumb;
      subj_uri = quad.s;
      pred_uri = quad.p;
      ...
      if (quad.o.type === RDF_object) {
        ...
        if (is_one_of(pred_uri, NAME_SYNS)) {
          ...
          if (this.make_nodes_for_literals) {
            ...
            if (objVal == null) {
              new ((function() {
                throw Error("missing value for " + JSON.stringify([subj_uri, pred_uri, quad.o]));
              })());
            }
    ...
    Huviz.prototype.init_settings_from_json = function() {
      var WidgetClass, a, control, control_name, control_spec, event_type, graph_control, input, inputElem, k, label, old_val, option, settings_input_sel, v, value, _i, _len, _ref, _ref1, _ref2, _ref3;
      ...
      this.settings = d3.select(this.args.settings_sel);
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        for (control_name in control_spec) {
          ...
          input = false;
          graph_control = this.settings.append('div').attr('id', control_name).attr('class', 'a_setting');
      ...
      this.settings.append('div').attr('class', 'buffer_space');
    };
