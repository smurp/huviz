...
(function() {
  ...
  Huviz = (function() {
    ...
    Huviz.prototype.generic_name_success_handler = function(data, textStatus, jqXHR, queryManager) {
      var error, resp_type, success_handler;
      try {
    ...
    Huviz.prototype.fetchAndShow = function(url, callback) {
      ...
      if (url.match(/.ttl/)) {
        ...
        the_parser = this.parseAndShowJSONLD;
      } else {
        msg = ("Could not load " + url + ". The data file format is not supported! ") + "Only files with .jsonld, .ttl and .nq extensions are accepted.";
        this.hide_state_msg();
      ...
      if (the_parser === this.parseAndShowJSONLD) {
        this.parseAndShowJSONLD(url, callback);
        return;
      }
    ...
    Huviz.prototype.big_go_button_onclick_sparql = function(event) {
      var endpoint_label_uri, foundUri, graphUri;
    ...
    Huviz.prototype.get_server_for_dataset = function(datasetUri) {
      var aUrl, domain, serverType, serverUrl;
    ...
    Huviz.prototype.render_visualization_source_display = function(vis_src_args) {
      ...
      if (dataset && ontology) {
        ...
        source_html = "<p><span class=\"dt_label\">Dataset:</span> " + dataset.label + "</p>\n<p><span class=\"dt_label\">Ontology:</span> " + ontology.label + "</p>";
      } else if (endpoint) {
        add_reload_button = true;
        source_html = "<p><span class=\"dt_label\">Endpoint:</span> " + endpoint.label + "</p>";
        if (endpoint.graph) {
          source_html += "<p><span class=\"dt_label\">Graph:</span> " + endpoint.graph.label + "</p>";
        }
        if (endpoint.item) {
          source_html += "<p><span class=\"dt_label\">Item:</span> " + endpoint.item.label + "</p>";
        }
        if (endpoint.limit) {
    ...
    Huviz.prototype.build_sparql_form = function() {
      var endpoint_labels_id, endpoint_limit_id, fromGraph, select_box, sparqlGraphSelectorId, sparqlQryInput_id;
      ...
      return this.endpoint_labels_JQElem.focusout(this.endpoint_labels__focusout);
    };
    ...
    function Huviz(incoming_args) {
      this.receive_jsonld = __bind(this.receive_jsonld, this);
      this.parseAndShowJSONLD = __bind(this.parseAndShowJSONLD, this);
    ...
    Huviz.prototype.parseAndShowJSONLD = function(uri) {
      ...
      worker.addEventListener('message', this.receive_jsonld);
      return worker.postMessage({
        url: aUri.toString()
      });
    };

    Huviz.prototype.receive_jsonld = function(event) {
      var _ref;
      console.log("receive_jsonld()", event);
      if ((_ref = event.data.type) === 'alert' || _ref === 'error') {
        return alert(event.data.data);
      }
    };
