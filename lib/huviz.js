...
(function() {
  ...
  SettingsWidget = (function() {
    function SettingsWidget(inputElem, state) {
  ...
  UsernameWidget = (function(_super) {
    ...
    UsernameWidget.prototype.set_state = function(state) {
      ...
      this.widgetJQElem.find('.stateIcon').attr('class', "stateIcon fa " + stateIcon);
      color = this.state_to_color[state];
      this.widgetJQElem.css('border-color', color);
      this.widgetJQElem.css('color', color);
    };

    UsernameWidget.prototype.can_run = function() {
      var _ref;
      return (_ref = this.state) === 'good' || _ref === 'untried';
    };
  ...
  GeoUserNameWidget = (function(_super) {
    ...
    function GeoUserNameWidget() {
      return GeoUserNameWidget.__super__.constructor.apply(this, arguments);
    }
  ...
  Huviz = (function() {
    ...
    Huviz.prototype.discover_geoname_name = function(aUrl) {
      var id, idInt, k2p, url, userId, widget;
      ...
      if ((widget = this.discover_geonames_as__widget)) {
        ...
        if (widget.state === 'good') {
          this.countdown_input('discover_geonames_remaining');
        }
      }
      ...
      $.ajax({
        ...
        success: (function(_this) {
          return function(json, textStatus, request) {
            var containershipQuad, deeperQuad, deeply, depth, geoNamesRoot, geoRec, greedily, key, msg, name, placeQuad, pred, quad, rem, seen_name, soughtGeoname, subj, theType, value, _i, _ref;
            if (json.status) {
            ...
            rem = _this.countdown_input('discover_geonames_remaining');
            console.log('discover_geonames_remaining', rem);
            if ((widget = _this.discover_geonames_as__widget)) {
              if (widget.state !== 'good') {
                _this.countdown_input('discover_geonames_remaining');
                _this.discover_geonames_as__widget.set_state('good');
                _this.discover_names('sws.geonames.org');
              }
            }
    ...
    Huviz.prototype.auto_discover = function(uri) {
      ...
      if (uri.startsWith("http://sws.geonames.org/") && this.discover_geonames_as__widget.can_run() && this.discover_geonames_remaining > 0) {
        ...
        for (control_name in control_spec) {
          ...
          if (control.input.type === 'select') {
            ...
            if (control.input != null) {
              ...
              if (WidgetClass && true) {
                inputElem = input[0][0];
                this[control_name + '__widget'] = new WidgetClass(inputElem);
              }
            }
