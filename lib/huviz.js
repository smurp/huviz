...
(function() {
  ...
  CommandController = (function() {
    ...
    CommandController.prototype.make_command_history = function() {
      ...
      this.scriptDownloadButton = this.scriptPlayerControls.append('button').attr('title', 'save script as .txt').attr('style', 'margin-left:1em').on('click', this.on_downloadscript_hybrid_clicked);
  ...
  Huviz = (function() {
    ...
    Huviz.prototype.ensure_dataset = function(dataset_rec, store_in_db) {
      ...
      if (dataset_rec.isOntology) {
        if (this.ontology_loader) {
          this.ontology_loader.add_resource(dataset_rec, store_in_db);
        }
      }
      if (this.dataset_loader && !dataset_rec.isEndpoint) {
        this.dataset_loader.add_resource(dataset_rec, store_in_db);
      }
      if (dataset_rec.isEndpoint && this.endpoint_loader) {
        return this.endpoint_loader.add_resource(dataset_rec, store_in_db);
      }
    };

    Huviz.prototype.add_resource_to_db = function(dataset_rec, callback) {
      var req, store, trx;
      ...
      trx.onerror = (function(_this) {
        return function(e) {
          console.log(e);
          return alert("add_resource(" + dataset_rec.uri + ") error!!!");
        };
      })(this);
        ...
        return function(why) {
          ...
          return function(event) {
            ...
            if (cursor) {
              ...
              } else if (rec.rsrcType === 'ontology' || legacyOntology) {
                _this.ontology_loader.add_resource_option(rec);
              } else if (rec.rsrcType === 'script') {
                _this.script_loader.add_resource_option(rec);
              } else if (rec.rsrcType === 'endpoint') {
    ...
    Huviz.prototype.init_dataset_menus = function() {
      ...
      if (!this.endpoint_loader && this.args.endpoint_loader__append_to_sel) {
        this.endpoint_loader = new PickOrProvide(this, this.args.endpoint_loader__append_to_sel, 'Sparql', 'EndpointPP', false, true, {
          rsrcType: 'endpoint'
        });
      }
      if (!this.script_loader && this.args.script_loader__append_to_sel) {
        this.script_loader = new PickOrProvide(this, this.args.script_loader__append_to_sel, 'Script', 'ScriptPP', false, false, {
    ...
    Huviz.prototype.visualize_dataset_using_ontology = function(ignoreEvent, dataset, ontologies) {
      ...
      if (this.script_loader.value) {
        alert('It is time to trigger the loading of the dataset and ontology OR endpoint ' + 'but the requisite info has not yet been saved in the script ' + 'on the other hand: if the user want to run the script on different data.... ' + 'then do we pause and let them mess with the script OR let them pick the data. ' + 'In fact, perhaps the thing to do is load the dataset and ontology into their ' + 'pickers so the user can mess with them before proceeding.  YES.  PERFECT.');
      }
    ...
    Huviz.prototype.load_script_from_JSON = function(json) {
      var cmdArgs, _i, _len, _results;
      alert('load_script_from_JSON');
      _results = [];
      for (_i = 0, _len = json.length; _i < _len; _i++) {
        cmdArgs = json[_i];
        _results.push(this.gclui.push_command_onto_history(this.gclui.new_GraphCommand(cmdArgs)));
      }
      return _results;
    };
    ...
    Huviz.prototype.parse_script_file = function(data, fname) {
      ...
      while (lines.length) {
        line = lines.shift();
        if (line.includes(this.json_marker)) {
  ...
  PickOrProvide = (function() {
    ...
    PickOrProvide.prototype.add_uri = function(uri_or_rec) {
      ...
      if (dataset_rec.rsrcType == null) {
        dataset_rec.rsrcType = this.opts.rsrcType;
      }
      this.add_resource(dataset_rec, true);
    ...
    PickOrProvide.prototype.add_resource = function(dataset_rec, store_in_db) {
      var uri;
      uri = dataset_rec.uri;
      if (store_in_db) {
        return this.huviz.add_resource_to_db(dataset_rec, this.add_resource_option);
      } else {
        return this.add_resource_option(dataset_rec);
      }
    };

    PickOrProvide.prototype.add_resource_option = function(dataset) {
      var uri;
      uri = dataset.uri;
        ...
        return function(evt) {
          ...
          try {
            if (filename.match(/.(txt|.json)$/)) {
              ...
              blurt(("Unknown file format. Unable to parse '" + filename + "'. ") + "Only .txt and .huviz files supported.", 'alert');
              _this.huviz.reset_dataset_ontology_loader();
              return $('.delete_option').attr('style', '');
            }
          } catch (_error) {
