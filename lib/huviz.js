...
(function() {
  ...
  NAME_SYNS = [FOAF_name, RDFS_label, 'rdfs:label', 'name', SKOS_prefLabel, XL_literalForm, SCHEMA_name, DCE_title];
  ...
  Huviz = (function() {
    ...
    Huviz.prototype.mousemove = function() {
      var action, cursor_text, e, edge, edit_state, pair, the_node, _i, _len, _ref, _ref1, _ref2, _ref3;
    ...
    Huviz.prototype.draw_self_edge_circle = function(cx, cy, strclr, length, line_width, e) {
      var end_angle, filclr, radius, special_focus, start_angle;
      ...
      end_angle = false;
      special_focus = false;
      return this.draw_circle(cx, cy, radius, strclr, filclr, start_angle, end_angle, special_focus);
    };
        ...
        _results.push((function() {
          ...
          for (_j = 0, _len1 = edges_between.length; _j < _len1; _j++) {
            ...
            if ((e.source.fisheye.x === e.target.fisheye.x) && (e.source.fisheye.y === e.target.fisheye.y)) {
              this.draw_self_edge_circle(e.source.fisheye.x, e.source.fisheye.y, e.color, e.contexts.length, line_width, e);
            } else {
    ...
    Huviz.prototype.discover_geoname_name = function(aUrl) {
      var id, idInt, k2p, rem, soughtId, url, userId, widget;
      ...
      $.ajax({
        ...
        success: (function(_this) {
          return function(json, textStatus, request) {
            var again, containershipQuad, deeperQuad, deeply, depth, geoNamesRoot, geoRec, greedily, key, msg, name, placeQuad, pred, quad, seen_name, soughtGeoname, state_at_start, subj, theType, value, _i, _ref;
            if (json.status) {
    ...
    Huviz.prototype.auto_discover_name_for = function(uri) {
      var aUrl, args, comment, config, domainName, downloadUrl, e, hasDomainName, retval, try_even_though_CORS_should_block, _ref, _ref1;
      if (uri.startsWith('_')) {
        return;
      }
      try {
      ...
      if (hasDomainName("vocab.getty.edu")) {
        if (try_even_though_CORS_should_block) {
          ...
          retval = this.ingest_quads_from(downloadUrl, this.discover_labels(uri));
          return;
        } else {
          args = {
            success_handler: this.name_tsv_success_handler,
            server_config: {
              source: 'http://vocab.getty.edu/sparql.tsv'
            }
          };
          this.run_sparql_name_query(uri, args);
          return;
        }
      }
      if (hasDomainName("geonames.org") && ((_ref = this.discover_geonames_as__widget.state) === 'untried' || _ref === 'looking' || _ref === 'good') && this.discover_geonames_remaining > 0) {
        this.discover_geoname_name(aUrl);
        return;
      }
      _ref1 = this.ldf_domain_configs;
      for (domainName in _ref1) {
        config = _ref1[domainName];
        if (domainName === '*') {
          this.run_ldf_name_query(uri, null, comment, config);
          return;
        } else if (hasDomainName(domainName)) {
          this.run_ldf_name_query(uri, null, comment, config);
          return;
        }
      }
    ...
    Huviz.prototype.run_sparql_name_query = function(namelessUri, args, comment) {
      var defaults, query, serverUri;
      if (args == null) {
        args = {};
      }
      query = "# " + (comment || ("run_ldf_name_query(" + namelessUri + ")")) + "\n" + this.make_name_query(namelessUri, args.expansion);
      defaults = {
        default_terms: {
          s: namelessUri,
          p: RDFS_label
      ...
      return this.run_managed_query_ajax(query, serverUri, args);
    };

    Huviz.prototype.name_tsv_success_handler = function(data, textStatus, jqXHR, queryManager) {
    ...
    Huviz.prototype.JUNK_PILE = function(wft) {
      var args, comment, handler, query, sparql, _ref;
      _ref = this.make_sparql_name_query_and_handler(uri, expansion), sparql = _ref[0], handler = _ref[1];
      args = {
        success_handler: handler
      };
      comment = "# " + (comment || ("run_sparql_name_query(" + uri + ")")) + "\n";
      query = comment + sparql;
      return this.run_managed_query_ajax(query, uri, args);
    };

    Huviz.prototype.ldf_domain_configs = {
      'dbpedia.org': {
    ...
    Huviz.prototype.make_name_query = function(uri) {
    ...
    Huviz.prototype.convert_bare_obj_to_GreenTurtle = function(bare_term) {
      var msg;
      if (bare_term[0] === '<') {
    ...
    Huviz.prototype.convert_bare_uri_to_string = function(bareUri) {
    ...
    Huviz.prototype.name_result_handler = function(result, queryManager) {
      var error, parseObj, parseUri, q, spo;
      if (queryManager.args.from_N3) {
        result.from_n3 = true;
        parseObj = this.convert_N3_obj_to_GreenTurtle;
        parseUri = this.convert_N3_uri_to_string;
      } else {
        parseObj = this.convert_bare_obj_to_GreenTurtle;
        parseUri = this.convert_bare_uri_to_string;
      }
      if ((spo = true)) {
        q = {
          s: parseUri(result['?s'] || queryManager.args.default_terms.s),
          p: parseUri(result['?p'] || queryManager.args.default_terms.p),
          o: parseObj(result['?o'])
        };
        try {
    ...
    Huviz.prototype.run_ldf_name_query = function(uri, expansion, comment, server_config) {
      var args, query;
      query = "# " + (comment || ("run_ldf_name_query(" + uri + ")")) + "\n" + this.make_name_query(uri, expansion);
      args = {
        result_handler: this.name_result_handler,
      ...
      return this.run_managed_query_ldf(query, uri, args);
    };

    Huviz.prototype.run_managed_query_ldf = function(qry, url, args) {
      var error_callback, ldf_worker, queryManager, result_handler, server_config, success_handler, timeout;
      if (args == null) {
        args = {};
      }
      if (args.worker == null) {
      ...
      queryManager = this.run_managed_query_abstract(qry, url, args);
      success_handler = args.success_handler, error_callback = args.error_callback, timeout = args.timeout, result_handler = args.result_handler, server_config = args.server_config;
      if (server_config == null) {
        server_config = {};
      }
      if (server_config.source == null) {
        server_config.source = "http://fragments.dbpedia.org/2016-04/en";
      }
      if (args.worker === 'comunica') {
          ...
          return function(event) {
            ...
            switch (type) {
              ...
              case 'error':
                ...
                return queryManager.setErrorColor();
              case 'end':
    ...
    Huviz.prototype.run_managed_query_abstract = function(qry, url, args) {
      var queryManager;
      if (args == null) {
        args = {};
      }
      ...
      queryManager = this.log_query_with_timeout(qry, args.timeout);
      queryManager.args = args;
      return queryManager;
    };

    Huviz.prototype.run_managed_query_ajax = function(qry, serverUrl, args) {
      var ajax_settings, error_callback, more, queryManager, success_handler, timeout;
      queryManager = this.run_managed_query_abstract(qry, serverUrl, args);
          ...
          return function(data, textStatus, jqXHR) {
            ...
            if (success_handler != null) {
              try {
                ...
                return queryManager.displayError(e);
              }
            }
          };
        })(this),
    ...
    Huviz.prototype.DEPRECATED_load_endpoint_data_and_show = function(subject, callback) {
      var note;
      ...
      worker.addEventListener('message', (function(_this) {
        return function(e) {
          ...
          if (e.data.method_name === 'log_query') {
            queryManager = _this.run_managed_query_abstract("#SPARQL_Worker\n" + e.data.qry, url, queryManagerArgs);
            return;
    ...
    Huviz.prototype.get_neighbors_via_sparql = function(chosen, callback) {
      ...
      if (!chosen.fully_loaded) {
        ...
        if (this.endpoint_loader.outstanding_requests < maxReq) {
    ...
    Huviz.prototype.choose = function(chosen, callback_after_choosing) {
      ...
      if (this.using_sparql() && !chosen.fully_loaded) {
    ...
    Huviz.prototype.init_resource_menus = function() {
      var dataset_selector, endpoint_selector, last_val, ontology_selector, script_selector, sel;
    ...
    function Huviz(incoming_args) {
      ...
      this.convert_bare_uri_to_string = __bind(this.convert_bare_uri_to_string, this);
      this.convert_N3_uri_to_string = __bind(this.convert_N3_uri_to_string, this);
      this.convert_bare_obj_to_GreenTurtle = __bind(this.convert_bare_obj_to_GreenTurtle, this);
      this.name_tsv_success_handler = __bind(this.name_tsv_success_handler, this);
  ...
  QueryManager = (function() {
    ...
    QueryManager.prototype.colorQuery = function(color) {
      return this.preJQElem.css('background', color);
    };

    QueryManager.prototype.setNoneColor = function() {
      return this.colorQuery('lightgrey');
    };

    QueryManager.prototype.setErrorColor = function() {
      return this.colorQuery('pink');
    };

    QueryManager.prototype.setSuccessColor = function() {
      return this.colorQuery('lightgreen');
    };

    QueryManager.prototype.setKilledColor = function() {
      this.setNoneColor();
      return this.preJQElem.css('color', 'white');
    };

    QueryManager.prototype.displayError = function(e) {
      console.warn(e);
      return this.qryJQElem.append("<div class=\"queryError\">" + e + "</div>");
    };

    QueryManager.prototype.displayResults = function(results) {
      return this.qryJQElem.append("<div class=\"queryResults\">" + results + "</div>");
    };
