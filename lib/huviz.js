...
(function(/*! Stitch !*/) {
  ...
  ColoredTreePicker = (function(_super) {
    ...
    ColoredTreePicker.prototype.recolor = function() {
      ...
      branch = this.elem.node().children[0];
    ...
    ColoredTreePicker.prototype.recolor_node = function(retval, recursor, id, elem_raw, indent) {
      ...
      if (this.is_abstract(id)) {
        ...
        if (verbose && ((_ref = recursor.i) === 1 || _ref === (recursor.count + 1))) {
...
(function() {
  ...
  CommandController = (function() {
    ...
    CommandController.prototype.handle_like_input = function(evt) {
      ...
      if (like_has_a_value) {
        this.huviz.set_search_regex(like_value);
    ...
    CommandController.prototype.hide_doit_button = function() {
      return $(this.doit_butt.node()).hide();
    };

    CommandController.prototype.show_doit_button = function() {
      return $(this.doit_butt.node()).show();
    };
    ...
    CommandController.prototype.build_command = function() {
      ...
      like_str = (this.like_input.node().value || "").trim();
      if (like_str) {
        args.like = like_str;
      }
    ...
    CommandController.prototype.prepare_command = function(cmd) {
      ...
      if (this.nextcommand_prompts_visible || true) {
        ...
        if (this.command.verb_phrase_ready) {
          $(this.nextcommand_verb_phrase.node()).addClass('nextcommand_prompt_ready').removeClass('nextcommand_prompt_unready');
        } else {
          $(this.nextcommand_verb_phrase.node()).removeClass('nextcommand_prompt_ready').addClass('nextcommand_prompt_unready');
        }
        this.nextcommand_noun_phrase.text(this.command.noun_phrase);
        if (this.command.noun_phrase_ready) {
          $(this.nextcommand_noun_phrase.node()).addClass('nextcommand_prompt_ready').removeClass('nextcommand_prompt_unready');
        } else {
          $(this.nextcommand_noun_phrase.node()).removeClass('nextcommand_prompt_ready').addClass('nextcommand_prompt_unready');
        }
  ...
  unpad_md = function(txt, pad) {
    var in_lines, line, out, out_lines, _i, _len;
    pad = "    ";
    out_lines = [];
    in_lines = txt.split("\n");
    for (_i = 0, _len = in_lines.length; _i < _len; _i++) {
      line = in_lines[_i];
  ...
  SettingsWidget = (function() {
    function SettingsWidget(huviz, inputElem, state) {
      this.huviz = huviz;
  ...
  UsernameWidget = (function(_super) {
    __extends(UsernameWidget, _super);
  ...
  GeoUserNameWidget = (function(_super) {
    __extends(GeoUserNameWidget, _super);
  ...
  Huviz = (function() {
    ...
    Huviz.prototype.how_heavy_are = function(n, label, cb) {
      var after, before, buncha, diff, k, m, memories, per, retval, _i;
      memories = [];
      ...
      for (m = _i = 1; 1 <= n ? _i <= n : _i >= n; m = 1 <= n ? ++_i : --_i) {
    ...
    Huviz.prototype.get_mouse_point = function(d3_event) {
      if (d3_event == null) {
        d3_event = this.mouse_receiver.node();
      }
    ...
    Huviz.prototype.mousemove = function() {
      var action, cursor_text, e, edge, edit_state, pair, _i, _len, _ref, _ref1, _ref2;
    ...
    Huviz.prototype.should_display_labels_as = function(which) {
    ...
    Huviz.prototype.render_node_info_box = function(info_node) {
      var all_names, color, color_headers, dialogArgs, id_display, link_from, name, names_all_langs, node_info_html, node_inspector_id, node_out_links, node_type, note, other_types, target, target_prefix, width, _i, _j, _k, _l, _len, _len1, _len2, _len3, _ref, _ref1, _ref2, _ref3;
      ...
      if (info_node.links_from.length > 0) {
        _ref1 = info_node.links_from;
        for (_k = 0, _len2 = _ref1.length; _k < _len2; _k++) {
          link_from = _ref1[_k];
          _ref2 = this.render_target_for_display(link_from.target), target_prefix = _ref2[0], target = _ref2[1];
    ...
    Huviz.prototype.init_sets = function() {
      ...
      this.matched_set = SortedSet().named("matched").sort_on("id").labelled(this.human_term.matched).sub_of(this.all_set).isFlag('matched');
    ...
    Huviz.prototype.update_d3forceManyBody = function() {
      console.warn("distanceMax", this.distanceMax);
      this.d3forceManyBody = d3.forceManyBody().strength(-50).distanceMax(this.distanceMax);
    };

    Huviz.prototype.d3simulation_shake = function(spazLevel) {
      if (spazLevel == null) {
        spazLevel = .4;
      }
      if (this.d3simulation != null) {
        return this.d3simulation.alpha(spazLevel).restart();
      }
    };

    Huviz.prototype.reset_svg = function() {
    ...
    Huviz.prototype.get_visible_subset = function(super_set) {
      var node, retlist, _i, _len;
      if (super_set == null) {
        super_set = this.all_set;
      }
      retlist = [];
      for (_i = 0, _len = super_set.length; _i < _len; _i++) {
        node = super_set[_i];
    ...
    Huviz.prototype.find_node_or_edge_closest_to_pointer_using_d3 = function() {
      var closest_node, x, y, _ref;
      if (this.should_display_labels_as('boxNGs')) {
        return;
      }
      _ref = this.last_mouse_pos, x = _ref[0], y = _ref[1];
      closest_node = this.d3simulation.find(x, y, this.focus_threshold);
      if (closest_node) {
        return this.set_focused_node(closest_node);
      }
    };
    ...
    Huviz.prototype.set_focused_node = function(node) {
      var svg_node;
      if (node) {
        console.debug("set_focused_node(" + node.id + ")", node.focused_node && "already");
      }
      if (node === false) {
    ...
    Huviz.prototype.draw_edges_from = function(node) {
      var arw_angle, draw_n_n, e, edge_width, edges_between, line_width, msg, n_n, num_edges, sway, x2, y2, _i, _len, _ref, _results;
        ...
        return this.graphed_set.forEach((function(_this) {
          return function(d, i) {
            ...
            if (_this.use_canvas) {
              ...
              if (!_this.should_display_labels_as('boxNGs')) {
                if (d.boxNG) {
                  _this.remove_boxNG(d);
                }
              }
              if (_this.should_display_labels_as('pills')) {
    ...
    Huviz.prototype.should_show_label = function(node) {
      return node.labelled || node.focused_edge || node.matched || (this.label_graphed && node.state === this.graphed_set) || dist_lt(this.last_mouse_pos, node, this.label_show_range);
    };
    ...
    Huviz.prototype.draw_labels = function() {
      ...
      if (this.use_canvas || this.use_webgl) {
        ...
        label_node = (function(_this) {
          return function(node) {
            var ctx, flip, flip_point, label, radians, textAlign;
            if (!_this.should_show_label(node)) {
              if (node.boxNG) {
                _this.remove_boxNG(node);
              }
              return;
            }
            ctx = _this.ctx;
            ...
            if (!_this.graphed_set.has(node) && _this.draw_lariat_labels_rotated) {
              ...
              if (_this.should_display_labels_as('pills')) {
                _this.update_canvas_pill(node, ctx);
              } else if (_this.should_display_labels_as('boxNGs')) {
                _this.update_boxNG(node);
              } else {
    ...
    Huviz.prototype.update_canvas_pill = function(node, ctx) {
      var adjust_x, adjust_y, alpha, cuts, fill, i, line_height, node_font_size, outline, pill_height, pill_width, print_label, radius, result, text, text_split, x, y, _i, _len;
    ...
    Huviz.prototype.remove_boxNG = function(node) {
      var elem;
      if ((elem = node.boxNG)) {
    ...
    Huviz.prototype.update_boxNG = function(node) {
      var elem, elemAppearsFocused, elemShouldAppearFocused, jqElem, needsFixing;
      ...
      elemShouldAppearFocused = node.focused_node || node.focused_edge;
      elemAppearsFocused = elem.className.includes('focusedNode');
      needsFixing = elemAppearsFocused !== elemShouldAppearFocused;
      if (needsFixing) {
    ...
    Huviz.prototype.move_boxNG_if_needed = function(node, elem, jqElem) {
      var fish_x, fish_y, nodeMoved;
    ...
    Huviz.prototype.style_boxNG_jqElem = function(node, jqElem, css) {
      var font_size;
      if (css == null) {
        css = {};
      }
    ...
    Huviz.prototype.focus_boxNG_jqElem = function(node, jqElem, css) {
      var font_size;
      jqElem.addClass('focusedNode');
      if (css == null) {
        css = {};
      }
      font_size = this.label_em * this.focused_mag;
      css['font-size'] = font_size + 'em';
      css['z-index'] = 10000;
      this.style_boxNG_jqElem(node, jqElem, css);
    };

    Huviz.prototype.unfocus_boxNG_jqElem = function(node, jqElem) {
      jqElem.removeClass('focusedNode');
    };

    Huviz.prototype.make_boxNG = function(node) {
      var elem;
      elem = this.addDivWithIdAndClasses(null, "boxNG", this.viscanvas_elem);
      elem.innerHTML = node.pretty_name;
      elem.onmousemove = (function(_this) {
        return function(evt) {
          return _this.mousemove_boxNG(evt, node);
        };
      })(this);
      elem.onmousedown = (function(_this) {
        return function(evt) {
          return _this.mousedown_boxNG(evt, node);
        };
      })(this);
      elem.onmouseout = (function(_this) {
        return function(evt) {
          return _this.mouseout_boxNG(evt, node);
        };
      })(this);
      elem.onmouseenter = (function(_this) {
        return function(evt) {
          return _this.mouseenter_boxNG(evt, node);
        };
      })(this);
      elem.onmouseup = (function(_this) {
        return function(evt) {
          return _this.mouseup_boxNG(evt, node);
        };
      })(this);
      return elem;
    };

    Huviz.prototype.mousemove_boxNG = function(evt, node) {
    ...
    Huviz.prototype.mousedown_boxNG = function(evt, node) {
      console.debug('mousedown_boxNG', node.id);
      d3.event = evt;
      this.mousedown();
    };

    Huviz.prototype.mouseout_boxNG = function(evt, node) {
    ...
    Huviz.prototype.mouseenter_boxNG = function(evt, node) {
    ...
    Huviz.prototype.mouseup_boxNG = function(evt, node) {
    ...
    Huviz.prototype.should_display_boxNG = function(node) {
      return (node.state === this.graphed_set || (node.focused_edge != null)) && this.is_in_viewport(node);
    };

    Huviz.prototype.is_in_viewport = function(node) {
      var _ref, _ref1;
      return ((0 < (_ref = node.fisheye.x) && _ref < this.width)) && ((0 < (_ref1 = node.fisheye.y) && _ref1 < this.height));
    };

    Huviz.prototype.set_boxNG_editability = function(node, truth) {
      if (truth) {
        ...
        return function(node) {
          ...
          if (node.focused_node || (node.focused_edge != null)) {
            if (_this.should_display_labels_as('pills')) {
              ...
              label = _this.scroll_pretty_name(node);
              if (node.state === _this.graphed_set) {
    ...
    Huviz.prototype.position_nodes_by_packing = function() {
      ...
      _results = [];
      while (++i < n) {
        _results.push(q.visit(this.position_node_by_packing(this.graphed_set[i])));
      }
      return _results;
    };
    ...
    Huviz.prototype.discovery_triple_ingestor_N3 = function(data, textStatus, request, discoArgs) {
      var parser, quadMunger, quadTester, quad_count, _ref;
      if (discoArgs == null) {
        discoArgs = {};
      }
      _ref = this.getTesterAndMunger(discoArgs), quadTester = _ref.quadTester, quadMunger = _ref.quadMunger;
      quad_count = 0;
      parser = N3.Parser();
      return parser.parse(data, (function(_this) {
        return function(err, quad, pref) {
          var aQuad, _i, _len, _ref1, _results;
          if (err && (discoArgs.onErr != null)) {
            discoArgs.onErr(err);
          }
          if (quadTester(quad)) {
            _ref1 = quadMunger(quad);
            _results = [];
            for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
              aQuad = _ref1[_i];
    ...
    Huviz.prototype.discovery_triple_ingestor_GreenTurtle = function(data, textStatus, request, discoArgs) {
      var aQuad, dataset, frame, graphUri, obj, pred, pred_id, quad, quadMunger, quadTester, subj_uri, _i, _j, _len, _len1, _ref, _ref1, _ref2, _ref3, _ref4;
      if (discoArgs == null) {
        discoArgs = {};
      }
      ...
      _ref1 = dataset.subjects;
      for (subj_uri in _ref1) {
        frame = _ref1[subj_uri];
        _ref2 = frame.predicates;
        for (pred_id in _ref2) {
          pred = _ref2[pred_id];
          _ref3 = pred.objects;
    ...
    Huviz.prototype.preset_discover_geonames_remaining = function() {
      var count, node, url, _i, _len, _ref;
      count = 0;
      ...
      return {
        ...
        success: (function(_this) {
          return function(json, textStatus, request) {
            var again, containershipQuad, deeperQuad, deeply, depth, geoNamesRoot, geoRec, greedily, key, msg, name, params, placeQuad, pred, quad, seen_name, soughtGeoname, state_at_start, subj, theType, value, widget, _i, _ref;
            if (json.status) {
      ...
      if (uris.length === 1) {
        ...
        subj_constraint = "FILTER (?s IN (" + ((function() {
          ...
          for (_i = 0, _len = uris.length; _i < _len; _i++) {
            uri = uris[_i];
            _results.push(this.deprefix(uri, prefix, expansion));
          }
          return _results;
        }).call(this)).join(', ') + "))";
      }
    ...
    Huviz.prototype.auto_discover_name_for = function(namelessUri, node) {
      var aUrl, args, downloadUrl, e, hasDomainName, retval, serverSpec, serverUrl, try_even_though_CORS_should_block, _ref;
      ...
      if (hasDomainName("geonames.org")) {
        ...
        if (((_ref = this.discover_geonames_as__widget.state) === 'untried' || _ref === 'looking' || _ref === 'good') && this.discover_geonames_remaining > 0) {
    ...
    Huviz.prototype.tsv_name_success_handler = function(data, textStatus, jqXHR, queryManager) {
      var cols, e, firstLine, line, lines, result_handler, row, rowJson, table, _i, _len;
    ...
    Huviz.prototype.json_name_success_handler = function(data, textStatus, jqXHR, queryManager) {
      var e, resultJson, result_handler, table, _i, _len, _ref;
      result_handler = queryManager.args.result_handler;
      try {
        table = [];
    ...
    Huviz.prototype.make_name_query = function(uri, in_args) {
      var args, lines, pred, pred_num, predicates, _i, _len;
    ...
    Huviz.prototype.consider_suppressing_edge_and_ends_re_OA = function(edge) {
      ...
      if (edge.predicate.lid === 'exact') {
        ...
        if (!(_ref = edge.target, __indexOf.call(this.hasAnnotation_targets, _ref) >= 0)) {
    ...
    Huviz.prototype.suppress_node_re_OA_if_appropriate = function(node) {
      var should_suppress, taxon, _i, _len, _ref;
    ...
    Huviz.prototype.infer_edge_end_types = function(edge) {
      var domain_lid, ranges, _base, _base1;
      if ((_base = edge.source).type == null) {
        _base.type = 'Thing';
      }
      if ((_base1 = edge.target).type == null) {
        _base1.type = 'Thing';
      }
      ...
      make_success_handler = (function(_this) {
        return function() {
          return function(data, textStatus, jqXHR, queryManager) {
            var graph, graph_options, graphsNotFound, json_check, json_data, label, results, _i, _len;
    ...
    Huviz.prototype.add_nodes_from_SPARQL = function(json_data, subject, queryManager) {
      var a_node, context, data, i, language, node, node_list_empty, node_not_in_list, obj_type, obj_val, plainLiteral, pred, q, results, snode, subj, _i, _j, _k, _len, _len1, _len2, _ref, _ref1, _results;
      data = '';
      context = this.get_context();
    ...
    Huviz.prototype.add_nodes_from_SPARQL_Worker = function(queryTarget, callback) {
      ...
      worker.addEventListener('message', (function(_this) {
        return function(e) {
          var a_node, add_fully_loaded, error, i, quad, _i, _j, _len, _len1, _ref, _ref1;
    ...
    Huviz.prototype.show_and_hide_links_from_node = function(node) {
    ...
    Huviz.prototype.add_matching_nodes_to_matched_set = function() {
      this.nodes.forEach((function(_this) {
        return function(node, i) {
          if (node.name.match(_this.search_regex)) {
            if (!node.matched) {
              return _this.matched_set.add(node);
            }
          } else {
            if (node.matched) {
              return _this.matched_set.remove(node);
            }
          }
        };
      })(this));
    ...
    Huviz.prototype.update_searchterm = function() {
      var text;
      text = this.gclui.like_input.text();
    ...
    Huviz.prototype.update_state = function(node) {
      ...
      if ((_ref = node.state) === this.discarded_set || _ref === this.hidden_set || _ref === this.shelved_set) {
        this.remove_boxNG(node);
        if (node.focused_node) {
          this.set_focused_node();
        }
    ...
    Huviz.prototype.update_d3links = function() {
      if (this.d3links != null) {
        return this.d3links.links(this.links_set);
      }
    };
    ...
    Huviz.prototype.snippet_position_str_to_obj = function(str) {
      var left, top, _ref;
      _ref = str.replace(new RegExp('([a-z]*)\\+', 'g'), '').split(' ').map(function(c) {
        return parseInt(c);
    ...
    Huviz.prototype.print_edge = function(edge) {
      var cb, context, context_no, edge_inspector_id, make_callback, me, _i, _len, _ref, _results;
      context_no = 0;
      _ref = edge.contexts;
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    ...
    Huviz.prototype.make_generic_query = function(in_args) {
      var args, lines, pattern, term, terms, val, _i, _len, _ref;
      args = this.compose_object_from_defaults_and_incoming(this.default_name_query_args, in_args);
    ...
    Huviz.prototype.init_gclc = function() {
      var dom_node, pid, _i, _len, _ref;
      this.gclc = new GraphCommandLanguageCtrl(this);
      this.init_resource_menus();
      if (this.gclui == null) {
        dom_node = d3.select(this.args.gclui_sel).node();
    ...
    Huviz.prototype.search_sparql_by_label = function(request, response) {
      ...
      make_success_handler = (function(_this) {
        return function() {
          return function(data, textStatus, jqXHR, queryManager) {
            var e, json_check, json_data, label, results, selections, this_result, _i, _len;
            json_check = typeof data;
            if (json_check === 'string') {
              try {
                json_data = JSON.parse(data);
    ...
    Huviz.prototype.make_tabs_html = function() {
      var firstClass, firstClass_, html, id, idSel, idx, jQElem_list, mkcb, t, tab_id, tab_specs, theDivs, theTabs, _i, _len;
      jQElem_list = [];
    ...
    Huviz.prototype.create_tabs = function() {
      var contentAreaJQElem, elem, html, jQElem_list, pair, tabKey, tabSel, _i, _len, _ref;
    ...
    function Huviz(incoming_args) {
      this.receive_quaff_lod = __bind(this.receive_quaff_lod, this);
      this.parseAndShowFile = __bind(this.parseAndShowFile, this);
      ...
      this.on_change_reset_settings_to_default = __bind(this.on_change_reset_settings_to_default, this);
      this.change_setting_to_from = __bind(this.change_setting_to_from, this);
      this.change_graph_settings = __bind(this.change_graph_settings, this);
      this.update_graph_settings = __bind(this.update_graph_settings, this);
      this.update_settings_cursor = __bind(this.update_settings_cursor, this);
      this.init_settings_to_defaults = __bind(this.init_settings_to_defaults, this);
      ...
      this.complete_construction = __bind(this.complete_construction, this);
      this.catch_reject_init_settings = __bind(this.catch_reject_init_settings, this);
      ...
      this.sparqlGraphSelector_onchange = __bind(this.sparqlGraphSelector_onchange, this);
      this.endpoint_labels__focusout = __bind(this.endpoint_labels__focusout, this);
      this.endpoint_labels__update = __bind(this.endpoint_labels__update, this);
      this.endpoint_labels__autocompleteselect = __bind(this.endpoint_labels__autocompleteselect, this);
      this.spo_query__update = __bind(this.spo_query__update, this);
      this.build_sparql_form = __bind(this.build_sparql_form, this);
      this.set_ontology_from_dataset_if_possible = __bind(this.set_ontology_from_dataset_if_possible, this);
      this.show_shareable_link_dialog = __bind(this.show_shareable_link_dialog, this);
      this.enable_go_button = __bind(this.enable_go_button, this);
      this.disable_go_button = __bind(this.disable_go_button, this);
      ...
      this.after_visualize_dataset_using_ontology = __bind(this.after_visualize_dataset_using_ontology, this);
      this.visualize_dataset_using_ontology = __bind(this.visualize_dataset_using_ontology, this);
      this.turn_off_loading_notice = __bind(this.turn_off_loading_notice, this);
      this.turn_on_loading_notice = __bind(this.turn_on_loading_notice, this);
      this.update_graph_form = __bind(this.update_graph_form, this);
      this.update_dataset_forms = __bind(this.update_dataset_forms, this);
      this.big_go_button_onclick = __bind(this.big_go_button_onclick, this);
      ...
      this.sparql_graph_query_and_show__trigger = __bind(this.sparql_graph_query_and_show__trigger, this);
      this.log_query = __bind(this.log_query, this);
      ...
      this.name_result_handler = __bind(this.name_result_handler, this);
      this.convert_obj_uri_to_string = __bind(this.convert_obj_uri_to_string, this);
      this.convert_obj_obj_to_GreenTurtle = __bind(this.convert_obj_obj_to_GreenTurtle, this);
      this.convert_str_uri_to_string = __bind(this.convert_str_uri_to_string, this);
      this.convert_N3_uri_to_string = __bind(this.convert_N3_uri_to_string, this);
      this.convert_str_obj_to_GreenTurtle = __bind(this.convert_str_obj_to_GreenTurtle, this);
      this.generic_name_success_handler = __bind(this.generic_name_success_handler, this);
      this.generic_success_handler = __bind(this.generic_success_handler, this);
      this.display_graph_success_handler = __bind(this.display_graph_success_handler, this);
      this.json_name_success_handler = __bind(this.json_name_success_handler, this);
      this.tsv_name_success_handler = __bind(this.tsv_name_success_handler, this);
      this.show_geonames_instructions = __bind(this.show_geonames_instructions, this);
      ...
      this.mouseup_boxNG = __bind(this.mouseup_boxNG, this);
      this.mouseenter_boxNG = __bind(this.mouseenter_boxNG, this);
      this.mouseout_boxNG = __bind(this.mouseout_boxNG, this);
      this.mousedown_boxNG = __bind(this.mousedown_boxNG, this);
      this.mousemove_boxNG = __bind(this.mousemove_boxNG, this);
      this.mouseover_of_huviz_controls = __bind(this.mouseover_of_huviz_controls, this);
      this.mouseout_of_huviz_controls = __bind(this.mouseout_of_huviz_controls, this);
      ...
      this.close_node_inspector = __bind(this.close_node_inspector, this);
      ...
      if ((_base = this.args).settings_sel == null) {
        _base.settings_sel = this.oldToUniqueTabSel['settings'];
      }
    ...
    Huviz.prototype.initialize_svg = function() {
    ...
    Huviz.prototype.initialize_d3_force_simulation = function() {
      ...
      return this.d3simulation.nodes([]).on('tick', this.tick);
    };

    Huviz.prototype.sim_restart = function() {
    ...
    Huviz.prototype.prepare_viscanvas = function() {
    ...
    Huviz.prototype.maybe_start_with_search_node = function() {
      if (this.start_with_search_node) {
        setTimeout((function(_this) {
          return function() {
            _this.collapse_tabs;
            _this.change_setting_to_from('display_labels_as', 'boxNGs');
            _this.change_setting_to_from('show_class_instance_edges', true);
            _this.make_search_node();
          };
        })(this));
      }
    };
    ...
    Huviz.prototype.add_search_node_quads = function() {
    ...
    Huviz.prototype.make_search_node = function() {
      var search_node;
      this.add_search_node_quads();
      search_node = this.all_set[0];
      return this.choose(search_node, (function(_this) {
        return function() {
          _this.label(search_node);
          return _this.set_boxNG_editability(search_node, true);
        };
      })(this));
    };
    ...
    Huviz.prototype.goto_tab = function(tab_id) {
      var tab_idx;
      tab_idx = this.tab_id_to_idx[tab_id];
      if (tab_idx == null) {
    ...
    Huviz.prototype.init_settings_to_defaults = function() {
      var WidgetClass, control, controlElem, control_name, control_spec, elemPromises, event_type, execInitElem, groupElem, groupName, initial_new_val, initial_old_val, inputElem, inputId, k, labelElem, opt, optIdx, optionElem, settings_input_sel, v, _i, _len, _ref, _ref1, _ref2, _ref3;
      elemPromises = [];
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        for (control_name in control_spec) {
          ...
          if (control.input != null) {
            ...
            if (!event_type) {
              if (((_ref3 = control.input.type) === 'checkbox' || _ref3 === 'range' || _ref3 === 'radio')) {
                event_type = 'input';
              } else {
                event_type = 'change';
              }
    ...
    Huviz.prototype.adjust_settings_from_list_of_specs = function(setting_specs) {
      var control, option, setting_name, setting_spec, value, _i, _j, _len, _len1, _ref;
      for (_i = 0, _len = setting_specs.length; _i < _len; _i++) {
        setting_spec = setting_specs[_i];
    ...
    Huviz.prototype.adjust_settings_from_kv_list = function(kv_list) {
      ...
      for (setting in kv_list) {
        ...
        _results.push(this.adjust_setting(setting, value));
      }
      return _results;
    };
    ...
    Huviz.prototype.on_change_distanceMax = function(val) {
      if (val === 1) {
    ...
    Huviz.prototype.on_change_display_labels_as = function(new_val, old_val) {
      ...
      if ((boxes_change_settings = false)) {
        if (new_val === 'pills' || new_val === 'boxNGs') {
    ...
    Huviz.prototype.pfm_update = function() {
      var calls_per_second, class_count, item, k, marker, marker_elem, new_count, noE, noN, noOR, noP, old_count, pfm_marker, pfm_marker_sel, time, v, val, _ref, _results;
      time = Date.now();
      class_count = 0;
      if (this.nodes) {
      ...
      for (pfm_marker in this.pfm_data) {
        ...
        if (marker.timed_count && (marker.timed_count.length > 0)) {
          ...
          if ((marker_elem = document.querySelector(pfm_marker_sel))) {
            _results.push(sparkline.sparkline(marker_elem, marker.timed_count));
          } else {
            throw new Error("#pfm_" + pfm_marker + " matches no elements");
          }
        } else if (marker.timed_count) {
          _results.push(marker.timed_count = [0.01]);
        } else {
          _results.push(void 0);
        }
      }
    ...
    Huviz.prototype.parseAndShowFile = function(uri, callback) {
      var aUri, error, fullUri, trigger_callback, worker;
      try {
        aUri = new URL(uri);
      } catch (_error) {
        error = _error;
      ...
      trigger_callback = (function(_this) {
        return function(event) {
          switch (event.data.type) {
            case 'end':
              _this.call_on_dataset_loaded();
              if (callback != null) {
                callback();
              }
              break;
            case 'error':
              _this.blurt(event.data.data, "error");
              break;
            default:
      ...
      worker.addEventListener('message', trigger_callback);
      worker.postMessage({
        url: aUri.toString()
      });
    };
    ...
    Huviz.prototype.receive_quaff_lod = function(event) {
      var graph, graph_uri, o, object, pred_uri, predicate, q, subj_uri, subject, _ref;
      _ref = event.data, subject = _ref.subject, predicate = _ref.predicate, object = _ref.object, graph = _ref.graph;
      if (!subject) {
        return;
      }
  ...
  Orlando = (function(_super) {
    ...
    function Orlando() {
      this.close_edge_inspector = __bind(this.close_edge_inspector, this);
    ...
    Orlando.prototype.push_snippet = function(msg_or_obj) {
      var dataType, dataType_curie, dataType_uri, dialogArgs, m, obj, obj_dd, pos, _ref;
      obj = msg_or_obj;
      if (true) {
    ...
    Orlando.prototype.clear_snippet_position_filled_for = function(match_id) {
      var id, pos, _ref;
      _ref = this.snippet_positions_filled;
      for (pos in _ref) {
        id = _ref[pos];
        if (id === match_id) {
  ...
  QueryManager = (function() {
    ...
    QueryManager.prototype.call_done_listeners = function() {
      ...
      while ((listener = this.listeners.shift())) {
        _results.push(setTimeout(listener, 10));
      }
      return _results;
    };
...
(function() {
  ...
  TreePicker = (function() {
    ...
    TreePicker.prototype.shield = function() {
      ...
      rect = d3.select(this.elem.node()).node().getBoundingClientRect();
      styles = {
        display: 'block',
        width: "" + rect.width + "px",
    ...
    TreePicker.prototype.get_container_elem_within_id = function(an_id) {
      var content_elem;
      content_elem = this.id_to_elem[an_id].node();
    ...
    TreePicker.prototype.add = function(new_id, parent_id, name, listener) {
      ...
      container = d3.select(this.get_or_create_container(parent).node());
