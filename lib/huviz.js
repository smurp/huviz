...
(function() {
  ...
  var GCLTest, GCLTestSuite, GraphCommand, GraphCommandLanguageCtrl,
    bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; };
  ...
  var BASE10, BASE57, CommandController, DCE_title, DC_subject, DragAndDropLoader, DragAndDropLoaderOfScripts, EditController, FOAF_Group, FOAF_Person, FOAF_name, GeoUserNameWidget, GraphCommandLanguageCtrl, GreenerTurtle, Huviz, IndexedDBService, IndexedDBStorageController, MANY_SPACES_REGEX, NAME_SYNS, OA_, OA_terms_regex, OSMT_name, OSMT_reg_name, OWL_Class, OWL_ObjectProperty, OWL_Thing, OntoViz, OntologicallyGrounded, Orlando, PEEKING_COLOR, PRIMORDIAL_ONTOLOGY, PickOrProvide, PickOrProvideScript, RDFS_label, RDF_Class, RDF_a, RDF_literal, RDF_object, RDF_subClassOf, RDF_type, SCHEMA_name, SKOS_prefLabel, SettingsWidget, Socrata, THUMB_PREDS, TYPE_SYNS, TextCursor, UNDEFINED, UsernameWidget, XL_literalForm, XML_TAG_REGEX, colorlog, convert, default_node_radius_policy, dist_lt, distance, escapeHtml, gcl, getPrefixedTypeSignature, getTypeSignature, has_predicate_value, has_type, hash, hpad, id_escape, ident, ids_to_show, int_to_base, is_a_main_node, is_node_to_always_show, is_one_of, linearize, node_radius_policies, noop, orlando_human_term, sel_to_id, start_with_http, strip_surrounding_quotes, synthIdFor, tau, themeStyles, typeSigRE, unescape_unicode, unique_id, unpad_md, wpad,
    extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
    hasProp = {}.hasOwnProperty,
    bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; };
