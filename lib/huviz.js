...
(function() {
  ...
  var CommandController, DC_subject, DragAndDropLoader, Edge, EditController, FOAF_Group, FOAF_Person, FOAF_name, GraphCommandLanguageCtrl, GreenerTurtle, Huviz, IndexedDBService, IndexedDBStorageController, MANY_SPACES_REGEX, NAME_SYNS, Node, OWL_Class, OWL_ObjectProperty, OWL_Thing, OntoViz, OntologicallyGrounded, Orlando, PEEKING_COLOR, PickOrProvide, Predicate, RDFS_label, RDF_a, RDF_literal, RDF_object, RDF_type, Socrata, TYPE_SYNS, Taxon, TextCursor, UNDEFINED, XML_TAG_REGEX, angliciser, blurt, case_insensitive_sort_on_name, default_node_radius_policy, dist_lt, distance, gcl, has_predicate_value, has_type, hpad, id_escape, ids_to_show, is_a_main_node, is_node_to_always_show, is_one_of, linearize, node_radius_policies, orlando_human_term, start_with_http, themeStyles, unescape_unicode, unique_id, uniquer, wpad,
    __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    __indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; },
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };
  ...
  case_insensitive_sort_on_name = function(a, b) {
    var na, nb;
    na = a.name ? a.name : " ";
    nb = b.name ? b.name : " ";
  ...
  Huviz = (function() {
    ...
    Huviz.prototype.init_sets = function() {
      ...
      this.shelved_set = SortedSet().sort_on(case_insensitive_sort_on_name).named("shelved").labelled(this.human_term.shelved).sub_of(this.all_set).isState();
      ...
      this.discarded_set = SortedSet().named("discarded").sort_on(case_insensitive_sort_on_name).labelled(this.human_term.discarded).sub_of(this.all_set).isState();
    ...
    Huviz.prototype.draw_nodes_in_set = function(set, radius, center) {
      ...
      return set.forEach((function(_this) {
        return function(node, i) {
          var rad, start;
          console.log("NODE ORDER --- " + _this.nodeOrderClockwise);
          start = 0;
          if (_this.nodeOrderClockwise) {
          ...
          if (_this.use_canvas) {
            _this.draw_circle(node.fisheye.x, node.fisheye.y, _this.calc_node_radius(node), node.color || "yellow", node.color || renderStyles.nodeHighlightOutline);
          }
          if (_this.use_webgl) {
    ...
    Huviz.prototype.on_change_theme_colors = function(new_val, old_val) {
      if (new_val) {
    ...
    Huviz.prototype.on_change_display_label_cartouches = function(new_val, old_val) {
      if (new_val) {
    ...
    Huviz.prototype.on_choose_node_display_order = function(new_val, old_val) {
      if (new_val) {
        this.nodeOrderClockwise = true;
      } else {
        this.nodeOrderClockwise = false;
      }
