...
(function() {
  ...
  CommandController = (function() {
    ...
    CommandController.prototype.make_command_history = function() {
      ...
      this.scriptRewindButton = this.scriptPlayerControls.append('button').append('i').attr("class", "fa fa-fast-backward").attr('title', 'rewind to start').on('click', this.on_rewind_click);
      this.scriptPlayButton = this.scriptPlayerControls.append('button').append('i').attr("class", "fa fa-play").attr('title', 'play script step by step').on('click', this.on_play_click);
      this.scriptForwardButton = this.scriptPlayerControls.append('button').append('i').attr("class", "fa fa-forward").attr('title', 'play script continuously').on('click', this.on_fastforward_click);
      this.cmdlist = history.append('div').attr('class', 'commandlist');
      return this.oldcommands = this.cmdlist.append('div').attr('id', 'commandhistory').style('max-height', "" + (this.huviz.height - 80) + "px");
    };
    ...
    CommandController.prototype.on_play_click = function() {
      ...
      _ref = this.cmd_history;
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        cmd = _ref[_i];
        _results.push(this.huviz.run_command(cmd));
      }
      return _results;
    };
