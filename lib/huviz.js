...
(function(/*! Stitch !*/) {
  ...
  EditController = (function() {
    ...
    EditController.prototype.create_edit_form = function(toggleEdit) {
      ...
      return toggleEdit.appendChild(formNode);
    };
    ...
    EditController.prototype.save_edit_form = function() {
      ...
      return this.proposed_edge = false;
    };

    EditController.prototype.clear_edit_form = function() {
      var form, i, inputFields, proposed_focused_edge, saveButton;
      form = this.controls;
      inputFields = form.getElementsByTagName('input');
      saveButton = form.getElementsByTagName('button')[0];
      for (i in inputFields) {
      ...
      if (this.proposed_edge) {
        console.log(this.proposed_edge);
        proposed_focused_edge = null;
        this.huviz.set_proposed_focused_edge(proposed_focused_edge);
        this.remove_proposed_edge(this.proposed_edge);
      }
    ...
    EditController.prototype.set_subject_node = function(node) {
      ...
      this.validate_proposed_edge(new_value, "s");
    };
    ...
    EditController.prototype.set_object_node = function(node) {
      ...
      if (new_value && this.deleted_last_edge) {
        this.deleted_last_edge = false;
    ...
    EditController.prototype.validate_proposed_edge = function() {
      ...
      if (subject_id && object_id) {
        ...
        console.log(q);
        console.log(this.proposed_edge);
        if ((this.proposed_edge != null) && q.o.value === this.proposed_edge.o.value) {
        ...
        return this.set_proposed_edge(q);
      }
    };

    EditController.prototype.set_proposed_edge = function(new_q) {
      console.log("creating a new proposed edge....");
      if (this.proposed_edge != null) {
        this.remove_proposed_edge(this.proposed_edge, new_q);
        return this.add_proposed_quad(new_q);
      } else {
        return this.add_proposed_quad(new_q);
      }
    };
    ...
    EditController.prototype.add_proposed_quad = function(q) {
      var new_proposed_edge, obj_n, pred_n, subj_n;
      console.log("I'm making a new quad.... " + q.s + " " + q.p + " " + q.o.value);
      this.huviz.add_quad(q);
      obj_n = this.huviz.get_or_create_node_by_id(q.o.value);
      subj_n = this.huviz.get_or_create_node_by_id(q.s);
      pred_n = this.huviz.get_or_create_predicate_by_id(q.p);
      new_proposed_edge = this.huviz.get_or_create_Edge(subj_n, obj_n, pred_n);
      this.huviz.set_proposed_focused_edge(new_proposed_edge);
      console.log(new_proposed_edge);
      this.huviz.show_link(new_proposed_edge);
      this.proposed_edge = q;
      return this.deleted_last_edge = true;
    };

    EditController.prototype.remove_proposed_edge = function(old_q, new_q) {
      var last_proposed_edge, obj_n, old_e, pred_n, subj_n;
      console.log("Removing previous proposed edge: " + old_q.s + " " + old_q.p + " " + old_q.o.value);
      obj_n = this.huviz.get_or_create_node_by_id(old_q.o.value);
      subj_n = this.huviz.get_or_create_node_by_id(old_q.s);
      pred_n = this.huviz.get_or_create_predicate_by_id(old_q.p);
      last_proposed_edge = this.huviz.get_or_create_Edge(subj_n, obj_n, pred_n);
      console.log(last_proposed_edge);
      console.log(this.graphed_set);
      old_e = old_q.s + " " + old_q.p + " " + old_q.o.value;
      this.huviz.remove_link(old_e);
      this.huviz.unshow_link(last_proposed_edge);
      this.deleted_last_edge = true;
      return delete this.huviz.edges_by_id[old_e];
    };
  ...
  Huviz = (function() {
    ...
    Huviz.prototype.find_node_or_edge_closest_to_pointer = function() {
      ...
      if (seeking === 'object_node') {
        if (this.editui.object_node !== this.object_node) {
    ...
    Huviz.prototype.set_focused_edge = function(new_focused_edge) {
      ...
      if (this.focused_edge !== new_focused_edge) {
        ...
        if (new_focused_edge != null) {
          ...
          return new_focused_edge.target.focused_edge = true;
        }
      }
    };

    Huviz.proposed_edge = false;

    Huviz.prototype.set_proposed_focused_edge = function(new_focused_edge) {
      console.log("Setting proposed focused edge...");
      if ((new_focused_edge != null) && this.focused_edge) {
        console.log("not nfe and fe");
        this.focused_edge.focused = false;
        delete this.focused_edge.source.focused_edge;
        return delete this.focused_edge.target.focused_edge;
      } else if (new_focused_edge) {
        console.log("yes, nfe");
        this.proposed_edge = true;
        return this.set_focused_edge(new_focused_edge);
      } else {
        this.proposed_edge = false;
        console.log("abort");
      }
    };
    ...
    Huviz.prototype.remove_link = function(e) {
      console.log(this.links_set.indexOf(e));
      console.log(e);
      console.log(this.links_set);
