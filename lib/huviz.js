 ...
 */
let FSMMixin = (superclass) => class extends superclass {
...
class FiniteStateMachine extends FSMMixin(Object) {}
...
function eval_ish$1(bod) { // calling this is equivalent to calling eval(bod)
  var f = new Function('return '+bod);
  return f();
}
  ...
  This method is intended to replace D3 constructions like:
   ...
   with
       ...
       const exp = append_html_to(
           ...
           domElem);
*/
function append_html_to(html, elem, position="beforeend") {
...
class TreePicker extends HTMLElement {
  static initClass() {
...
of the classes indicated.`,
  ...
  make_command_history() {
    ...
    this.cmdlist = history.
      append('div').
      attr('class','commandlist shrunk inset_shadow');
...
class Predicate extends TreeCtrl {
  ...
  constructor(id) {
    super(id);
    this.lid = uniquer$1(this.id); // lid means local_id
...
class PickOrProvide {
  ...
  constructor(huviz, append_to_sel, label, css_class, isOntology, isEndpoint, opts) {
    ...
    this.uniq_id = unique_id$1();
    this.select_id = unique_id$1();
    this.pickable_uid = unique_id$1();
    this.your_own_uid = unique_id$1();
...
`;
  ...
  constructor(huviz, append_to_sel, picker) {
    ...
    this.picker = picker;
    this.local_file_form_id = unique_id$1();
...
let DatasetDBMixin = (superclass) => class extends superclass {
  init_datasetDB() {
    const {
      indexedDB
  ...
  get_or_create_sel_for_picker(specificSel) {
    ...
    if ((sel == null)) {
      if ((this.pickersSel == null)) {
        ...
        const pickersId = unique_id$1('pickers_');
  ...
  init_resource_menus(args) {
    ...
    if (this.ontology_loader && !this.big_go_button) {
      this.big_go_button_id = unique_id$1('goButton_');
  ...
  build_sparql_form() {
    this.sparqlId = unique_id$1();
    const sparqlQryInput_id = `sparqlQryInput_${this.sparqlId}`;
    this.sparqlQryInput_selector = "#" + sparqlQryInput_id;
    const endpoint_limit_id = unique_id$1('endpoint_limit_');
    const endpoint_labels_id = unique_id$1('endpoint_labels_');
    const spo_query_id = unique_id$1('spo_query_');
...
class DatasetDB extends DatasetDBMixin(Object) {}

var stateMachineTTL = `

`;

class PickOrProvidePanel extends FSMMixin(HTMLElement) {
  constructor() {
    super();
...
class ResourceMenu extends DatasetDBMixin(FSMMixin(HTMLElement)) {
  constructor() {
    super();
...
SERVICE wikibase:label {
  ...
  add_quad(quad, sprql_subj) {  //sprq_sbj only used in SPARQL quieries
    ...
    const subj_lid = uniquer$1(subj_uri);  // FIXME rename uniquer to make_dom_safe_id
  ...
  try_to_set_node_type(node, type_uri) {
    ...
    const type_lid = uniquer$1(type_uri); // should ensure uniqueness
    if (!node._types) {
      node._types = [];
    }
  ...
  get_or_create_node_by_id(uri, name, isLiteral) {
    ...
    if (node.lid == null) { node.lid = uniquer$1(node.id); }
    if ((node.name == null)) {
  ...
  hatch(node) {
    ...
    node.lid = uniquer$1(node.id); // FIXME ensure uniqueness
...
`;
  ...
  make_default_args() {
    ...
    return {
      add_to_HVZ: true,
      ctrl_handle_sel: unique_id$1('#ctrl_handle_'),
      gclui_sel: unique_id$1('#gclui_'),
      git_base_url: "https://github.com/smurp/huviz/commit/",
      hide_fullscreen_button: false,
      huviz_top_sel: unique_id$1('#huviz_top_'), // if not provided then create
      make_pickers: true,
      performance_dashboard_sel: unique_id$1('#performance_dashboard_'),
      settings: {},
      show_edit: false,
      show_tabs: true,
      skip_log_tick: true,
      state_msg_box_sel: unique_id$1('#state_msg_box_'),
      status_sel: unique_id$1('#status_'),
      stay_square: false,
      tab_specs: ['commands','settings','history'], // things break if these are not present
      tabs_minWidth: 300,
      use_old_tab_ids: false,
      viscanvas_sel: unique_id$1('#viscanvas_'),
      vissvg_sel: unique_id$1('#vissvg_')};
  }
  ...
  init_settings_to_defaults() {
    ...
    for (let control_spec of this.default_settings) {
      for (var control_name in control_spec) {
        ...
        const inputId = unique_id$1(control_name+'_');
  ...
  parseTTLOntology(data, textStatus) {
    ...
    if ((typeof GreenerTurtle !== 'undefined' && GreenerTurtle !== null) &&
      ...
      for (let subj_uri in this.raw_ontology.subjects) {
        ...
        const subj_lid = uniquer$1(subj_uri);
        for (let pred_id in frame.predicates) {
          const pred = frame.predicates[pred_id];
          const pred_lid = uniquer$1(pred_id);
          for (let obj of pred.objects) {
            ...
            const obj_lid = uniquer$1(obj_raw);
