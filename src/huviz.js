...
SERVICE wikibase:label {
  ...
  parseAndShowTTLData(data, textStatus, callback) {
    ...
    throw new Error('parseAndShowTTLData has been replaced by parseAndShowAnyData');
...
`;
  ...
  parseAndShowAnyUri(uri, callback) {
    ...
    worker.postMessage({action: 'fetchUrl', url: aUri.toString()});
  }
    ...
    worker.addEventListener('message', (event) => {
      ...
      default: // ie no type
        if (event.data.subject) {
          ...
          console.log(
            `trigger_callback(event) not handling event.data.type:${event.data.type} event.data:`,
            event.data);
        }
      }
    });
    worker.postMessage({action: 'readData', ext: rsrc.ext, theDataToRead: data});
  }
