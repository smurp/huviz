...
class TreePicker {
  ...
  shield() {
    ...
    const rect = d3.select(this.elem[0][0]).node().getBoundingClientRect();
    const styles = {
      display: 'block',
      width: `${rect.width}px`,
  ...
  get_container_elem_within_id(an_id) {
    ...
    const content_elem = this.id_to_elem[an_id][0][0];
  ...
  resort_recursively(an_id) {
    ...
    for (let child_id of Array.from(kids_ids)) {
    ...
    for (let val_elem_pair of Array.from(val_elem_pairs)) {
      ...
      for (let node_id in tree) {
        ...
        if (rest.length > 1) {
          ...
          if (top && this.extra_classes) {
            for (let css_class of Array.from(this.extra_classes)) {
              my_contents.classed(css_class, true);
            }
          }
          result.push(this.show_tree(rest[1], my_contents, listener, false));
        } else {
  ...
  effect_click(id, new_state, send_leafward, listener, args) {
    if (send_leafward) {
      ...
      if (kids != null) {
        for (let child_id of Array.from(kids)) {
          if (child_id !== id) {
    ...
    if (listener != null) {  // TODO(shawn) replace with custom event?
      const elem = this.id_to_elem[id];
      return listener.call(this, id, new_state, elem, args); // now this==picker not the event
    }
  }
  ...
  add(new_id, parent_id, name, listener) {
    ...
    const container = d3.select(this.get_or_create_container(parent)[0][0]);
    if (this.needs_expander) {
      this.get_or_create_expander(parent,parent_id);
    }
    return this.show_tree(branch, container, listener); // 0x25b6
  }
  get_or_create_expander(thing, id) {
    if ((thing != null) && thing) {
      ...
      return exp.on('click', () => { // TODO: make this function a method on the class
        d3.event.stopPropagation();
        const id2 = exp[0][0].parentNode.parentNode.getAttribute("id");
        if (id2 !== id) {
  ...
  expand_all() {
    return (() => {
      ...
      for (let id in this.id_is_collapsed) {
        ...
        if (collapsed) {
          result.push(this.expand_by_id(id));
        } else {
          result.push(undefined);
        }
      }
  ...
  calc_new_indirect_state(id) {
    ...
    for (let child_id of Array.from(this.id_to_children[id])) {
